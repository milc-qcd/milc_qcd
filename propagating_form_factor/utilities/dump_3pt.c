/*
 *
 *  Dump the from factor to standard output
 *
 *
 *
 */
      ...
      int no_q_values, int no_p_values, int no_oper, int nt, int nocopies)
{
  ...
  int corr_stride ; 
  /************************************************************/
  corr_stride = FORM_WHERE(t,zonk_pt,seq_pt,spect_pt,q_pt,p_pt,oper_pt) ; 
		...
		for(oper_pt = 0 ; oper_pt < no_oper ; ++oper_pt)
		  {
		    ...
		    for(t = 0 ; t < nt ; ++t)
		      {
			where = copy_pt*corr_stride + FORM_WHERE(t,zonk_pt,seq_pt,spect_pt,q_pt,p_pt,oper_pt) ;
  ...
  for(zonk_pt = 0 ; zonk_pt < no_zonked ; ++zonk_pt )
  {
	...
	for(vel_pt = 0 ; vel_pt < novel ; ++vel_pt)
	{
	  ...
	  for(oper_pt = 0 ; oper_pt < no_oper ; ++oper_pt)
	  {
	    ...
	    for(t = 0 ; t < nt ; ++t)
	    {
	      where = FORM_WHERE(t,zonk_pt,spect_pt,q_pt,vel_pt,oper_pt) ;
