...
void dump_hl_twopt(complex *corr, int corr_oper_list[], int corr_copy_list[],
		   ...
		   int no_q_values, int no_oper, int nt, int nocopies)
{
  ...
  corr_stride = HL_TWOPT_FORM_WHERE(nt,zonk_pt ,spect_pt,q_pt, oper_pt) ; ; 
  ...
  for(zonk_pt = 0 ; zonk_pt < no_zonked_heavy ; ++zonk_pt )
  {
	...
	for(copy_pt = 0 ; copy_pt < nocopies ; ++copy_pt)
	  for(oper_pt = 0 ; oper_pt < no_oper ; ++oper_pt)
	  {
	    ...
	    for(t = 0 ; t < nt ; ++t)
	    {
	      where = corr_stride*copy_pt + HL_TWOPT_FORM_WHERE(t,zonk_pt ,spect_pt,q_pt, oper_pt) ;
...
void dump_ll_twopt(complex *corr,  int corr_oper_list[], int corr_copy_list[],
		   ...
		   int no_q_values, int no_oper, int nt)
{
  ...
  printf("Here is the light-light two point function\n");
  ...
  for(zonk_pt = 0 ; zonk_pt < no_zonked_light ; ++zonk_pt )
  {
	...
	printf("   Q--Momentum pointer = %d \n",q_pt  ); 
	  for(oper_pt = 0 ; oper_pt < no_oper ; ++oper_pt)
	  {
	    printf("       Operator pointer = %d  name = %s\n",
		   corr_oper_list[oper_pt], 
		   two_oper_name(corr_oper_list[oper_pt]) );
	    for(t = 0 ; t < nt ; ++t)
	    {
	      where = LL_TWOPT_FORM_WHERE(t,zonk_pt ,spect_pt,q_pt, oper_pt) ;
	      printf("         corr[ %d ] = ( %g , %g )\n",t , (corr+where)->real, (corr+where)->imag);
	    }
