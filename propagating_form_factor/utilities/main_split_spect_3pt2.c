/*
   ...
   spectator quarks and split it into separate files according to
   spectator quark.  This version uses more memory.
...
int main(int argc, char *argv[])
{
  ...
  if( argc < 4 )
  {
    printf("usage::  %s  <input 3pt file> <output file for 1st spect> <output file for 2nd spect> [< output file for 3rd spect>]\n",argv[0]);
    exit(1);
  }
  ...
  write_3pt_onespect(corr, 
		   output1_file, spect_select, q_momentum, p_momentum, hl_flag,
  ...
  spect_select = 1;
  write_3pt_onespect(corr, 
		   output2_file, spect_select, q_momentum, p_momentum, hl_flag,
		   no_zonked , no_spectator,  no_sequential,
		   no_p_mom, no_q_mom, no_oper, nt, nocopies)  ;
  ...
  spect_select = 2;
  write_3pt_onespect(corr, 
		   output3_file, spect_select, q_momentum, p_momentum, hl_flag,
		   no_zonked , no_spectator,  no_sequential,
		   no_p_mom, no_q_mom, no_oper, nt, nocopies)  ;
