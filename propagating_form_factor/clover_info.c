...
/* WARNING! This routine is included only to allow a complete
   compilation in propagating_form_factor.  At the moment we have
   not provided for writing propagator files from this code,
   so the source parameters source_r0, source_x, etc used here
   are not being defined properly by the higher level code. 
   CD 11/23/97 */
