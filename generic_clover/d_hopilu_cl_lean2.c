...
/* Modifications:
   ...
   8/02/01 Uses dslash_special - CD
    ...
    )
{
  ...
  dslash_special(mp, mp, PLUS, EVEN, tage, is_startede);
  is_startede = 1;
  ...
  for( N_iter = 1; N_iter < MaxHOP; N_iter = N_iter + 1) {
    ...
    /* mp_o = D_oe r_e */
    dslash_special(r, mp, PLUS, ODD, tago, is_startedo);
    is_startedo = 1;
    /* tmp_o = 1/R_o D_oe r_e */
    mult_ldu(mp, tmp, ODD);
    /* mp_e = D_eo/R_o D_oe r_e */
    dslash_special(tmp, mp, PLUS, EVEN, tage, is_startede);
    is_startede = 1;
  ...
  /* mp_o = D_oe * dest_e */
  dslash_special(dest, mp, PLUS, ODD, tago, is_startedo);
