...
/* Modifications ...
   ...
   1/20/97 Use multiple mass algorithm BJ 
           NOTE: MAX_KAP == 6 is implicitly used */
...
/* note: the vector of mem[0] must not be used by the d_bicgilu_lean.c 
    ...
    for (k = 0;k < 6;k++)
      {
	dest[k] = F_OFFSET(quark_prop2.c[k/4].d[(k % 4)]);
	mem[k]  = F_OFFSET(quark_prop2.c[(k+6)/4].d[((k+6)%4)]);
      }
   ...
   mem[8] = F_OFFSET(mp);
	...
	for(color=0;color<3;color++){
	  ...
	  for(spin=0;spin<4;spin++){
	    ...
	    source_r0 = r0[0];
	    w_source(F_OFFSET(chi),color,spin,wallflag,0,0,0,0,source_r0);
...
*/
	...
	for (k=0;k < num_kap;k++) 
      ...
      /*******************************************************************/
      /* FROM HERE ON, EVERYTHING IS THE SAME AS IN control_w_hl.c       */
      /*******************************************************************/
      /* note that quark_propagator does not contain the propagator 
         it is reloaded from disk anyways */
	...
	for(k=0;k<num_kap;k++){
		...
		kappa=kap[j];
		width=r0[0];
		fb_scr[j] = open_r_fast_quarkdump(scratch_file[j],num_prop);

#ifdef IOTIME
		dtime = -dclock();
#endif
		for(color=0;color<3;color++) for(spin=0;spin<4;spin++){
		    fast_quarkread(fb_scr[j], spin, color,
...
#endif
		close_r_fast_quarkdump(fb_scr[j],scratch_file[j]); 
		...
		if(this_node==0) 
		  {
		    printf("Time for hl mesons %d %d %e\n",j,k,dtime);
		    fflush(stdout);
		  }
#endif
...
#endif
	    for(color=0;color<3;color++) for(spin=0;spin<4;spin++){
		fast_quarkdump(fb_scr[k], spin, color,
		    F_OFFSET(quark_propagator.c[color].d[spin])); 
	    }
