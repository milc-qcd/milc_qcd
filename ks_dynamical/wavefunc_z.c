...
/* This version is for "screening wavefunctions".  It is like wavefunc_t
   except that z and t are interchanged. */
...
void permute_combine(field_offset src,field_offset space,int size,int dir);
    ...
    for(color=0;color<3;color++){ /* Make wall source */
	    ...
	    if( s->x%2==0 && s->y%2==0 && s->t%2==0 && s->z==0 ){
	    /* Antiperiodic b.c. in time direction make different source
		appropriate */
		x = cos( PI*(double)(s->t)/(double)nt );
	        s->phi.c[color] = cmplx(-x,0.0);
	    }
        }
    ...
    /* construct the diquark propagator--uses tempmat.e[0] and do this before
...
/* MESON CODE */
	...
	0,0,1	(-1)^t		rho-b1 (VT), gamma_t polarization ??!!??
	1,1,0	(-1)^(x+y)	rho-a1 (PV), gamma_t polarization
	1,0,1	(-1)^(x+t)	rho-a1 (PV), gamma_y polarization
	0,1,1	(-1)^(y+t)	rho-a1 (PV), gamma_x polarization
    ...
    */
    ...
    if(nx==ny && ny==nt){	/* average over permutations, one interchange */
	permute_combine( F_OFFSET(ttt.c[0]), F_OFFSET(cg_p.c[0]),
        ...
        if(nx==ny || ny==nt || nx==nt )
	     symmetry_combine( F_OFFSET(ttt.c[0]),
	    F_OFFSET(cg_p.c[0]), sizeof(complex), switch_dir);
	    /* combine with x <-> y site, y <-> t or x <-> t */
    }
    for(i=XUP;i<=TUP;i++)if(i!=ZUP){	/* average over coord <-> -coord */
        symmetry_combine( F_OFFSET(ttt.c[0]),
	    F_OFFSET(cg_p.c[0]), sizeof(complex), minus_block_dir[i]);
    }
    /* z <-> nz-z uses ordinary minus_map */
    symmetry_combine( F_OFFSET(ttt.c[0]),
	F_OFFSET(cg_p.c[0]), sizeof(complex), minus_dir[ZUP]);

    /* Dump wavefunction */
    zmin = 0; zmax=nz/2;
    scale = 1.0/(sym_factor*(nx/2)*(ny/2)*(nt/2));
	/* symmetries and volume factor from FFT's */
    FORALLSITES(i,s){
	CMULREAL( s->ttt.c[0], scale, s->ttt.c[0] );
    }
    write_wf( F_OFFSET(ttt.c[0]), "WF:",zmin,zmax);
 /* END MESONS CODE */
...
/* BARYON CODE */
    ...
    /* z <-> nz-z uses ordinary minus_map (commented out for now)
    symmetry_combine( F_OFFSET(ttt.c[0]),
	F_OFFSET(cg_p.c[0]), sizeof(complex), minus_dir[ZUP]);
*/
    /* Dump wavefunction */
    ...
    write_wf( F_OFFSET(ttt.c[0]), "BARW:",zmin,zmax);
...
void setup_block_fourier(int *key );
    ...
    key[XUP]=1; key[YUP]=1; key[ZUP]=0; key[TUP]=1;
	...
	key[i]=1;
	if(i!=ZUP) minus_block_dir[i] = make_gather( minus_block_map, key,
...
/* y <- x,  t <- y,  x <- t for forwards mapping.  "arg" is ignored. */
void permute_map(int x,int y,int z,int t,int *arg,
		 int fb,int *xp,int *yp,int *zp,int *tp)
{
    if(fb==FORWARDS){
...
void write_wf(field_offset src,char *string,int zmin,int zmax)
/* this subroutine writes a wave function from z-time zmin to zmax */
/* The string is a tag which identifies the output lines */
/* Output format:  STRING  z   x y t   real  imag  */
{
