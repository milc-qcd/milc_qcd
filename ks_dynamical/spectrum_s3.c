...
/* Spectrum for Kogut-Susskind pointlike hadrons, POINT SOURCE */
/* Need point source for susceptibility */
...
int spectrum() /* return the C.G. iteration number */
{
  ...
  for(z=0; z<nz; z++)
	...
	for(icol=0;icol<3;icol++)
      ...
      g_floatsum( &pi2prop );
      if(mynode()==0)printf("MES_P_SCREEN  %d  %e  %e  %e  %e  %e  %e\n",z,
  ...
  for(z=0; z<nz; z++)
      ...
      for(x=0;x<nx;x+=2)for(y=0;y<ny;y+=2)for(t=0;t<nt;t+=2)
	{
      ...
      g_floatsum( &barprop );
      if(mynode()==0)printf("NUC_P_SCREEN  %d  %e\n",z,(double)barprop);
    } /* nz-loop */
  ...
  for(z=0; z<nz; z++){
   ...
   for(z=0; z<nz-1; z++){
      if(mynode()==0)printf("QM_P_PROP  %d  %e\n",z,
