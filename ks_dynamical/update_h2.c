/****** update_h2.c  -- update the momentum matrices ******************/
/* MIMD version 6 */
/* Return maximum fermion force magnitude */
/* NOT MAINTAINED.  TEST BEFORE USE! */

#include "ks_dyn_includes.h"
void gauge_force( Real eps );
void fermion_force_2( Real eps );

Real update_h( Real eps ) {
Real ff,fermion_force_2( Real eps );
    /* gauge field force */
    gauge_force(eps);
    /* fermionic force */
...
dtime = -dclock();**/
	...
	wait_gather(tag0);
	FOREVENSITES(i,st){
	    mult_su3_mat_vec( &(st->link[dir]), gen_pt[0][i], &tvec);
...
/** END TEMP **/
	...
	wait_gather(tag1);
	FORODDSITES(i,st){
	    mult_su3_mat_vec( &(st->link[dir]), gen_pt[1][i], &tvec);
...
return( (Real)sqrt( (double)ff_max ) );
}
