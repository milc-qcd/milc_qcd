...
static void m_by_omega(wilson_propagator *wp, su3_vector *ksp, site *s,
		int ks_source_r[]){
  ...
  for(s0 = 0; s0 < 4; s0++)
    for(c0 = 0; c0 < 3; c0++)
      for(c1 = 0; c1 < 3; c1++)
	wp->c[c0].d[s0].d[s0].c[c1] = ksp[c0].c[c1];
...
void convert_ksprop_to_wprop(wilson_propagator *wp, su3_vector *ksp, int r[])
{
  int i;
  site *s;
