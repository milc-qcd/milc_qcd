    ...
    )
{
  ...
  /*  p = [L^(-1)*M*U^(-1)]_dag*r  */
  dslash_w_site_special(r,mmp,MINUS,ODD,tago,is_startedo);
  is_startedo = 1;
  dslash_w_site_special(mmp,p,MINUS,EVEN,tage,is_startede);
  is_startede = 1;
  ...
  for( N_iter = 0; N_iter < MinCG || (N_iter < MaxCG && RsdCG < qic->size_r); 
    ...
    dslash_w_site_special(r,mmp,MINUS,ODD,tago,is_startedo);
    is_startedo = 1;
    dslash_w_site_special(mmp,mmp,MINUS,EVEN,tage,is_startede);
    is_startede = 1;
    /*   cp = |mmp|^2  */
    cp=0.0;
    FOREVENSITES(i,s) {
