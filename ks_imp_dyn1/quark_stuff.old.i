...
void load_longlinks() {
  ...
  for (dir= 0 ; dir<= 3 ; dir++){  
    ...
    for( i =0, s =lattice; i <sites_on_node; i ++, s ++) clear_su3mat( &(s->longlink[dir]) );
	...
	for( i =0, s =lattice; i <sites_on_node; i ++, s ++) {
	  su3_adjoint( &(s->tempmat1), &(s->staple) );
          scalar_mult_add_su3_matrix( &(s->longlink[dir]),
	    &(s->staple), -q_paths[ipath].coeff,  &(s->longlink[dir]) );
...
void compute_gen_staple(field_offset staple, int mu, int nu, 
  ...
  if(staple!= -1 ){ 
    for( i =0, s =lattice; i <sites_on_node; i ++, s ++) {
      ...
      scalar_mult_add_su3_matrix(&(s->fatlink[mu]),
