...
void w_loop1(int tot_smear) {
  ...
  su3_matrix *s_link, *s_link_f, *t_link_f;
    ...
    nth = max_t; nxh = (nz>ny?ny>nx?nx:ny:nz)/2;
    ...
    if(s_link == NULL){
      fprintf(stderr,"w_loop1: CAN'T MALLOC s_link\n");
      fflush(stderr);
      terminate(1);
    }
    ...
    s_link_f = 
      (su3_matrix *)malloc(sites_on_node*sizeof(su3_matrix));
    if(s_link_f == NULL){
      fprintf(stderr,"w_loop1: CAN'T MALLOC s_link_f\n");
      fflush(stderr);
      terminate(1);
    }
    ...
    t_link_f = 
      (su3_matrix *)malloc(sites_on_node*sizeof(su3_matrix));
    if(t_link_f == NULL){
      fprintf(stderr,"w_loop1: CAN'T MALLOC t_link_f\n");
      fflush(stderr);
      terminate(1);
    }
	...
	for(r=0;r<nxh;r++){
		...
		FORALLSITES_OMP(i,s, private(tmat1,tmat2) reduction(+:wils_loop1[r+nxh*t]) ){
