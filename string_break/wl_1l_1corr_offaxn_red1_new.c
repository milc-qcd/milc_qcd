	...
	for(r=0;r<2;r++){
		...
		TUP, EVENANDODD, gen_pt[TUP] );
           ...
           /*  gather light prop     */
            ...
            gmtag = start_general_gather_site( F_OFFSET(dtmpvecs[1].n[0]),
		...
		/* Finally, compute the Wilson loops. */
                FORALLSITES(i,s){
                   if( ((s->t)+t+1)>=nt ){
                    ...
                    mult_su3_mat_vec(&tmat2,&(s->tempvec[0]),&(s->tempvec[1]));
                   }
                   else
                   {
                    mult_su3_mat_vec(&(s->s_link_f), &(s->tempvec[0]),
                       &(s->tempvec[1]));
                    }
                     cc = su3_dot(&(s->g_rand[j]),&(s->tempvec[1]));
		...
		/* Finally, compute the Wilson loops. */
                FORALLSITES(i,s){
                    if( ((s->t)+t+1)>=nt ){
                     mult_su3_nn( &(s->link[TUP]), &(s->s_link_f), &tmat1);
                     mult_su3_na( &tmat1, &(s->t_link_f), &tmat2);
                     mult_su3_mat_vec(&tmat2,&(s->resid),&(s->tempvec[1]));
                    }
                    else
                    {
                     mult_su3_mat_vec(&(s->s_link_f), &(s->resid),
                       &(s->tempvec[1]));
                    }
                     cc = su3_dot(&(s->g_rand[j]),&(s->tempvec[1]));
                     wils_loop2[r+r_off+nrmax*t+nrmax*nth*j].real -= cc.real;
...
/*
            if(this_node == 0)
            printf("WL_1LC1_OA_%d_OP_%d %d %d %d %e %e\n", step,tot_smear,j,r,
...
*/
	...
	for(r=0;r<nrmax;r++){
           ...
           if(this_node == 0)
            printf("WL_1LC1_OA_T_%d_OP_%d %d %d %e %e\n", step,tot_smear,r,t,
