...
//
// Description: measure singlet and non-singlet SU(3) densities at
...
void measure() {
   ...
   */
   ...
   CDIVREAL(plp_even,(Real)(nx*ny*nz/2),plp_even);
   CDIVREAL(plp_odd ,(Real)(nx*ny*nz/2),plp_odd);

   zphase = ce_itheta(phase);
   if(this_node == 0) {
      printf("ZMES\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\n",
	zphase.real, zphase.imag, density.real, density.imag,
        plp_even.real, plp_even.imag, plp_odd.real, plp_odd.imag);
