...
#include "ks_deflation_includes.h"//new
...
int readin(int prompt) {
  ...
  if(this_node==0){
    ...
    /*EigCG related parameters (move it down later):*/
    IF_OK status += get_i(stdin, prompt,"number_of_eigenvectors", &param.Nvecs );
    IF_OK status += get_i(stdin, prompt,"total_number_of_RHS", &param.nrhs );
    /* maximum no. of rhs in the incremental stage: */
    IF_OK status += get_i(stdin, prompt,"max_number_of_RHS_in_incr_stage", &param.inc_nrhs );

    IF_OK status += get_f(stdin, prompt,"initcg_restart_tolerance",
                              &param.restart_tol );
