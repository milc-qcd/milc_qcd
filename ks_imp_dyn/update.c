...
int update()  {
  ...
  for(step=1; step <= steps; step++){
    ...
    if(step==1){
      ...
      grsource_imp( F_OFFSET(phi1), mass1, EVEN, fn);
    ...
    restore_fermion_links_from_site(fn_links, MILC_PRECISION);
    fn = get_fm_links(fn_links, 0);
    imp_ferm_links_t *fn_next = get_fm_links(fn_links, 1);
    if(n == 2){
      iters += ks_congrad( F_OFFSET(phi1), F_OFFSET(xxx1), mass1,
			   niter, nrestart, rsqmin, MILC_PRECISION, EVEN, &final_rsq, fn );
      iters += ks_congrad( F_OFFSET(phi2), F_OFFSET(xxx2), mass2,
			   niter, nrestart, rsqmin, MILC_PRECISION, EVEN, &final_rsq, fn_next );
    } else {
    ...
    destroy_fn_links(fn_next);
    fn_last = get_fm_links(fn_links, n-1);
    dslash_site( F_OFFSET(xxx1), F_OFFSET(xxx1), ODD, fn);
    dslash_site( F_OFFSET(xxx2), F_OFFSET(xxx2), ODD, fn_last);
    destroy_fn_links(fn);
    destroy_fn_links(fn_last);
