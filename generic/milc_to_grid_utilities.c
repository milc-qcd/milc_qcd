...
#if 0
/*! \brief Constructor to create the single global grid_env_t object. */ 
void
create_grid_env ( int nx
                 , int ny
                 , int nz
                 , int nt
                 , int ncores
              ...
              )
{
    ...
    grid_env_t *env = (grid_env_t*)malloc(sizeof(grid_env_t));
    if(env == NULL) {
    ...
    g_grid_env_obj = env;    
    is_grid_env_setup = true;
}              

/*! \brief Destroy the global grid_env object */
void
destroy_grid_env (void)
{
    freeKS(g_grid_env_obj->ks_src1);
    freeKS(g_grid_env_obj->ks_dest1);
    freeKS(g_grid_env_obj->ks_src2);
    freeKS(g_grid_env_obj->ks_dest2);
    freeGauge(g_grid_env_obj->gll[0]);
    freeGauge(g_grid_env_obj->gll[1]);
    freeGauge18(g_grid_env_obj->gfl[0]);
    freeGauge18(g_grid_env_obj->gfl[1]);
    
    free(g_grid_env_obj);
    is_grid_env_setup = false;
}
...
get_links_from_lattice (void* fgauge[2], void *lgauge[2], fn_links_t *fn)
{
    ...
    //    FORSOMEPARITY(i,s,parity) {
	    ...
	    //if(ii != i) printf("YYY ii=%d not same as  i=%d\n", ii, i); 
            /* plug the 4 su3_matrices in thee forward dirs into the grid array */
