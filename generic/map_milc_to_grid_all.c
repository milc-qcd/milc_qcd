...
MILCTYPE * \
create_grid_raw4_##P##_##T (void){ \
  MILCTYPE *raw = NULL; \
  raw = (MILCTYPE *)malloc(4*sites_on_node*sizeof(MILCTYPE)); \
  if(raw == NULL){				\
    printf("create4_grid_raw: No room\n");	\
    return NULL;				\
  } \
...
void \
destroy_grid_raw4_##P##_##T (MILCTYPE *raw){ \
  free(raw); \
}
...
MILCTYPE * \
create_grid_raw_##P##_##T(void){ \
  MILCTYPE *raw = NULL; \
  raw = (MILCTYPE *)malloc(sites_on_node*sizeof(MILCTYPE)); \
  if(raw == NULL){ \
    printf("create_grid_raw: No room\n"); \
    return NULL; \
  } \
...
void \
destroy_grid_raw_##P##_##T (MILCTYPE *raw){ \
...
static void check_layout_compatibility(void){
  ...
  FORALLSITES_OMP(i,s,private(coords)){
    ...
    if(GRID_node_number_raw(coords) != this_node){
      printf("ERROR on node(%d): incompatible GRID_node_number in layout\n", this_node);
      terminate(1);
    }
  } END_LOOP_OMP;
 ...
 create_hash_##T(int milc_parity){				\
  ...
  if(hash_created_##T){					\
     ...
     FORALLSITES_OMP(i,s,private(j,coords)){					\
       site_coords(coords,s);						\
       j = GRID_node_index_raw_##T(coords, milc2grid_parity(milc_parity)); \
       hash_m2q_##T[i] = j;							\
       hash_q2m_##T[j] = i;							\
     } END_LOOP_OMP;							\
     hash_created_##T = 1;						\
...
RAWTYPE * \
create_grid_raw4_##P##_##T##_from_site(field_offset src, int milc_parity){ \
  int i,j,dir,status=0; \
  site *s; \
  RAWTYPE *raw; \
  MILC_SRCTYPE *tmp; \
  raw = create_grid_raw4_##P##_##T (); \
...
RAWTYPE * \
create_grid_raw4_##P##_##T##_from_field(MILC_SRCTYPE *src, int milc_parity){ \
  int i,j,dir,status=0;	 \
  site *s; \
  RAWTYPE *raw = NULL; \
  MILC_SRCTYPE *tmp; \
...
RAWTYPE * \
create_grid_raw_##P##_##T##_from_site(field_offset src, int milc_parity){ \
  int i,j,status=0; \
  site *s; \
  RAWTYPE *raw; \
  MILC_SRCTYPE *tmp; \
  raw = create_grid_raw_##P##_##T(); \
  if(raw == NULL)return NULL; \
  check_layout_compatibility(); \
  create_hash_##T(milc_parity);					\
...
RAWTYPE * \
create_grid_raw_##P##_##T##_from_field(MILC_SRCTYPE *src, int milc_parity){ \
  int i,j,status=0; \
  site *s; \
  RAWTYPE *raw; \
  raw = create_grid_raw_##P##_##T(); \
...
/* Map MILC gauge field in site structure to GRID through raw type */
/* Note, this version assumes four contiguous matrices per site */

#define make_create_from_site4(P, T, GRIDTYPE, RAWTYPE, MILCFLOAT) \
GRIDTYPE * \
create_grid_##P##_##T##_from_site4(field_offset src, int milc_parity){ \
  RAWTYPE *raw; \
  GRIDTYPE *grid; \
  raw = create_grid_raw4_##P##_##T##_from_site(src, milc_parity); \
  if(raw == NULL)terminate(1); \
  grid = GRID_##P##3_create_##T##_from_raw((MILCFLOAT *)raw, \
           milc2grid_parity(milc_parity)); \
  destroy_grid_raw4_##P##_##T(raw); raw = NULL; \
  return grid; \
}

/* Map MILC field to GRID through raw type */
/* Note, this version assumes four contiguous matrices per site */

#define make_create_from_field4(P, T, GRIDTYPE, RAWTYPE, MILC_SRCTYPE, MILCFLOAT) \
GRIDTYPE * \
create_grid_##P##_##T##_from_field4(MILC_SRCTYPE *src, int milc_parity){ \
  RAWTYPE *raw; \
  GRIDTYPE *grid; \
  raw = create_grid_raw4_##P##_##T##_from_field(src, milc_parity); \
  if(raw == NULL)terminate(1); \
  grid = GRID_##P##3_create_##T##_from_raw((MILCFLOAT *)raw, \
           milc2grid_parity(milc_parity)); \
  destroy_grid_raw4_##P##_##T(raw); raw = NULL; \
...
/* Map MILC site to GRID through raw type */

#define make_create_from_site(P, T, GRIDTYPE, RAWTYPE, MILCFLOAT) \
GRIDTYPE * \
create_grid_##P##_##T##_from_site(field_offset src, int milc_parity){ \
  RAWTYPE *raw; \
  GRIDTYPE *grid; \
  raw = create_grid_raw_##P##_##T##_from_site(src, milc_parity); \
  if(raw == NULL)terminate(1); \
  grid = GRID_##P##3_create_##T##_from_raw((MILCFLOAT *)raw, \
           milc2grid_parity(milc_parity)); \
  destroy_grid_raw_##P##_##T(raw); raw = NULL; \
  return grid; \
}

/* Map MILC field to GRID through raw type */

#define make_create_from_field(P, T, GRIDTYPE, RAWTYPE, MILC_SRCTYPE, MILCFLOAT) \
GRIDTYPE * \
create_grid_##P##_##T##_from_field(MILC_SRCTYPE *src, int milc_parity){ \
  RAWTYPE *raw; \
  GRIDTYPE *grid; \
  raw = create_grid_raw_##P##_##T##_from_field(src, milc_parity); \
  if(raw == NULL)terminate(1); \
  grid = GRID_##P##3_create_##T##_from_raw((MILCFLOAT *)raw, \
           milc2grid_parity(milc_parity)); \
  destroy_grid_raw_##P##_##T(raw); raw = NULL; \
...
void \
unload_grid_raw4_##P##_##T##_to_site(field_offset dest, RAWTYPE *raw, \
         int milc_parity){ \
  int i,j,dir,status=0;	 \
  site *s; \
  MILC_DSTTYPE *tmp; \
...
void \
unload_grid_raw4_##P##_##T##_to_field(MILC_DSTTYPE *dest, RAWTYPE *raw, \
         int milc_parity){ \
  int i,j,dir,status=0;	 \
  site *s; \
  MILC_DSTTYPE *tmp; \
...
void \
unload_grid_raw_##P##_##T##_to_site(field_offset dest, RAWTYPE *raw, \
       int milc_parity){ \
  int i,j,status=0; \
  site *s; \
  check_layout_compatibility(); \
...
void \
unload_grid_raw_##P##_##T##_to_field(MILC_DSTTYPE *dest, RAWTYPE *raw, \
       int milc_parity){ \
  int i,j,status=0;  \
  site *s; \
  check_layout_compatibility(); \
...
#define make_unload_to_site4(P, T, GRIDTYPE, RAWTYPE, MILCFLOAT) \
void \
unload_grid_##P##_##T##_to_site4(field_offset dest, GRIDTYPE *grid, int milc_parity){ \
  RAWTYPE **raw; \
  raw = create_grid_raw4_##P##_##T(); \
  if(raw == NULL)terminate(1); \
  GRID_##P##3_extract_##T##_to_raw((MILCFLOAT *)raw, grid, \
           milc2grid_parity(milc_parity)); \
  unload_grid_raw4_##P##_##T##_to_site(dest, raw, milc_parity); \
  destroy_grid_raw4_##P##_##T(raw); raw = NULL; \
  return; \
}
...
#define make_unload_to_field4(P, T, GRIDTYPE, RAWTYPE, MILC_DSTTYPE, MILCFLOAT) \
void \
 unload_grid_##P##_##T##_to_field4(MILC_DSTTYPE *dest, GRIDTYPE *grid, int milc_parity){ \
  RAWTYPE *raw; \
  raw = create_grid_raw4_##P##_##T(); \
  if(raw == NULL)terminate(1); \
  GRID_##P##3_extract_##T##_to_raw((MILCFLOAT *)raw, grid, \
           milc2grid_parity(milc_parity)); \
  unload_grid_raw4_##P##_##T##_to_field(dest, raw, milc_parity); \
  destroy_grid_raw4_##P##_##T(raw); raw = NULL; \
  return; \
}

/* Map color vector from GRID field to site */

#define make_unload_to_site(P, T, TYPE, MILCTYPE, MILCFLOAT) \
void \
unload_grid_##P##_##T##_to_site( field_offset dest, TYPE *grid, int parity){ \
  MILCTYPE *raw; \
  raw = create_grid_raw_##P##_##T(); \
  GRID_##P##3_extract_##T##_to_raw((MILCFLOAT *)raw, grid, milc2grid_parity(parity)); \
  unload_grid_raw_##P##_##T##_to_site(dest, raw, parity); \
  destroy_grid_raw_##P##_##T(raw); raw = NULL; \
}

/* Map color vector from GRID field to MILC field */

#define make_unload_to_field(P, T, TYPE, MILCTYPE, MILC_DSTTYPE, MILCFLOAT) \
void \
unload_grid_##P##_##T##_to_field( MILC_DSTTYPE *dest, TYPE *grid, int parity){ \
  MILCTYPE *raw; \
  raw = create_grid_raw_##P##_##T(); \
  GRID_##P##3_extract_##T##_to_raw((MILCFLOAT *)raw, grid, milc2grid_parity(parity)); \
  unload_grid_raw_##P##_##T##_to_field(dest, raw, parity); \
  destroy_grid_raw_##P##_##T(raw); raw = NULL; \
}

#if 0
/* Generate GRID asqtad links from gauge field in site structure */

#define make_create_L_from_site_gauge(P, TYPE, MILCTYPE, MILCFLOAT) \
TYPE * \
create_grid_##P##_L_from_site_gauge( GRID_info_t *info, \
    GRID_asqtad_coeffs_t *coeffs, field_offset src, int parity) \
{ \
  MILCTYPE *raw; \
  TYPE *grid; \
  GRID_##P##3_GaugeField *gauge; \
  raw = create_grid_raw4_##P##_G_from_site(src, parity); \
  if(raw == NULL)terminate(1); \
  gauge = GRID_##P##3_create_G_from_raw((MILCFLOAT *)raw, \
				  milc2grid_parity(parity)); \
  destroy_grid_raw4_##P##_G(raw); raw = NULL; \
  grid = GRID_##P##3_asqtad_create_L_from_G(info, coeffs, gauge); \
  GRID_##P##3_destroy_G(gauge); gauge = NULL; \
  return grid; \
}
#endif

/* Map preconstructed fat and long gauge fields from MILC site to GRID field */
/* THIS IS INCOMPLETE !!! DO NOT USE */

#define make_create_L_from_sites(P, TYPE, MILCTYPE, MILCFLOAT) \
TYPE * \
create_grid_##P##_L_from_sites( field_offset fat, field_offset lng, \
   int parity) \
{ \
  MILCTYPE *rawfat; \
  MILCTYPE *rawlng; \
  TYPE *grid; \
  rawfat = create_grid_raw4_##P##_G_from_site(fat, parity); \
  if(rawfat == NULL)terminate(1); \
  rawlng = create_grid_raw4_##P##_G_from_site(lng, parity); \
...
/* Map preconstructed fat and long gauge fields from MILC field to GRID field */

#define make_create_L_from_fields(P, TYPE, MILCTYPE, MILC_SRC_TYPE, MILCFLOAT) \
TYPE * \
create_grid_##P##_L_from_fields( MILC_SRC_TYPE *fat, MILC_SRC_TYPE *lng, \
...
{ \
  ...
  MILCTYPE *rawlngback; \
  TYPE *grid; \
  rawfat = create_grid_raw4_##P##_G_from_field(fat, parity); \
  if(rawfat == NULL)terminate(1); \
  rawlng = create_grid_raw4_##P##_G_from_field(lng, parity); \
  if(rawlng == NULL)terminate(1); \
  rawfatback = create_grid_raw4_##P##_G_from_field(fatback, parity); \
  ...
  destroy_grid_raw4_##P##_G(rawlngback); rawlngback = NULL; \
  return grid; \
}

/* Extract MILC fat link and long link fields from GRID asqtad links */

#define make_unload_L_to_fields(P, TYPE, MILCTYPE, MILC_DST_TYPE, MILCFLOAT) \
void \
unload_grid_##P##_L_to_fields( MILC_DST_TYPE *fat, MILC_DST_TYPE *lng, TYPE* grid, \
   int parity) \
{ \
  MILCTYPE *rawfat; \
  MILCTYPE *rawlng; \
  rawfat  = create_grid_raw4_##P##_G(); \
  if(rawfat == NULL)terminate(1); \
  rawlng = create_grid_raw4_##P##_G(); \
  if(rawlng == NULL)terminate(1); \
  GRID_##P##3_asqtad_extract_L_to_raw((MILCFLOAT *)rawfat, \
          (MILCFLOAT *)rawlng, grid, milc2grid_parity(parity)); \
  unload_grid_raw4_##P##_G_to_field(fat, rawfat, parity); \
  if(lng != NULL) \
  unload_grid_raw4_##P##_G_to_field(lng, rawlng, parity); \
  destroy_grid_raw4_##P##_G(rawfat); rawfat = NULL; \
  destroy_grid_raw4_##P##_G(rawlng); rawlng = NULL; \
  return; \
}
...
/* Extract MILC fat link and long link fields from GRID HISQ links */

#define make_unload_hisq_L_to_fields(P, TYPE, MILCTYPE, MILC_DST_TYPE, MILCFLOAT) \
void \
unload_grid_##P##_hisq_L_to_fields( MILC_DST_TYPE *fat, MILC_DST_TYPE *lng, TYPE* grid, \
   int parity) \
{ \
  MILCTYPE *rawfat; \
  MILCTYPE *rawlng; \
  ...
  rawlng = create_grid_raw4_##P##_G(); \
  if(rawlng == NULL)terminate(1); \
  GRID_##P##3_hisq_extract_L_to_raw((MILCFLOAT *)rawfat, \
          (MILCFLOAT *)rawlng, grid, milc2grid_parity(parity)); \
  unload_grid_raw4_##P##_G_to_field(fat, rawfat, parity); \
  if(lng != NULL) \
  unload_grid_raw4_##P##_G_to_field(lng, rawlng, parity); \
...
/* Map MILC clover term to raw GRID clover */
/* UNTESTED! */
...
void map_milc_clov_to_grid_raw_##P(MILCFLOAT *raw_clov, clover *milc_clov ){\
  int i,j,c,status=0;  \
  site *s;\
  MILCFLOAT *r;\
\
