...
/* 8/27/23 Version 7.8 port JHW, using staple field instead of site*/
...
msg_tag *mtag[4];
	...
	/* Allocate space for staples */
	su3_matrix *staple = 
		(su3_matrix *)malloc(sites_on_node*sizeof(su3_matrix));
	if(staple == NULL){
		fprintf(stderr,"ax_gauge: CAN'T MALLOC staple\n");
		fflush(stderr);
		terminate(1);
	}
		...
		for(j=0; j<3; j++)  {
			for(k=0; k<3; k++)  {
				if(j != k)  {
					...
					staple[i].e[j][k].real = 1.0;
					staple[i].e[j][k].imag = 0.0;
				}
			}
		}
	}
	...
	for(t=1;t<nt;t++){
		...
		FORALLSITES(i,s) if( (s->t)==t ){
			su3mat_copy( (su3_matrix *)(gen_pt[TUP][i]), &(staple[i]) );
			su3mat_copy( &(s->link[TUP]), &tmat);
			mult_su3_nn( &(staple[i]), &tmat, &(s->link[TUP]));
		}
	...
	for(dir=XUP;dir<=ZUP;dir++){
		mtag[dir] = start_gather_field( staple, sizeof(su3_matrix),
	...
	for(dir=XUP;dir<=ZUP;dir++){
		FORALLSITES(i,s){
			...
			mult_su3_nn( &(staple[i]), &tmat, &(s->link[dir]));
		}
