...
initialize_machine(int *argc, char ***argv)
{
  ...
  node0_printf("com_mpi: setting required thread-safety level to QMP_THREAD_MULTIPLE = %d\n", QMP_THREAD_MULTIPLE);
#else
  required = QMP_THREAD_SINGLE;
  node0_printf("com_mpi: setting required thread-safety level to QMP_THREAD_SINGLE = %d\n", QMP_THREAD_SINGLE);
#endif
  ...
  node0_printf("com_qmp: required thread-safety level %d can't be provided %d.\n", required, provided);
    fflush(stdout);
    terminate(1);
  }
