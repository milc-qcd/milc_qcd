...
*/
    ...
    for(j=0;j<3;j++){
	FORALLSITES(i,s){
	    t_clov_diag[i].di[0][j] = 
		-((su3_matrix *)F_PT(s,f_mn))->e[j][j].imag;
	    t_clov_diag[i].di[0][j+3] = 
		((su3_matrix *)F_PT(s,f_mn))->e[j][j].imag;
	    t_clov_diag[i].di[1][j] = 
		-((su3_matrix *)F_PT(s,f_mn))->e[j][j].imag;
	    t_clov_diag[i].di[1][j+3] = 
		((su3_matrix *)F_PT(s,f_mn))->e[j][j].imag;
	}
	jk2=(j+3)*(j+2)/2+3;
	for(k=0;k<j;k++){
	...
	for(k=0;k<j;k++){
		...
		CADD( t_clov[i].tr[0][jk], ctmp, t_clov[i].tr[0][jk]);
		CSUB( t_clov[i].tr[0][jk2], ctmp, t_clov[i].tr[0][jk2]);
		CSUB( t_clov[i].tr[1][jk], ctmp, t_clov[i].tr[1][jk]);
		CADD( t_clov[i].tr[1][jk2], ctmp, t_clov[i].tr[1][jk2]);
	    }
	    jk++;
	    jk2++;
	}
	...
	for(k=0;k<3;k++){
	    FORALLSITES(i,s){
		CSUB( t_clov[i].tr[0][jk],
		    ((su3_matrix *)F_PT(s,f_mn))->e[j][k], t_clov[i].tr[0][jk]);
		CSUB( t_clov[i].tr[1][jk],
		    ((su3_matrix *)F_PT(s,f_mn))->e[j][k], t_clov[i].tr[1][jk]);
	    }
	    jk++;
	}
    }
