...
void write_appl_gauge_info(FILE *fp, gauge_file *gf)
{
  ...
  write_gauge_info_item(fp,"gauge.fix.description","%s",
			param.gauge_fix_description,0,0);
  write_gauge_info_item(fp,"gauge.fix.tolerance","%e",
			(char *)&param.gauge_fix_tol,0,0);
  write_gauge_info_item(fp,"gauge.translate","%d",
			(char *)param.rshift,4,sizeof(int));
...
char *create_MILC_info(){
  ...
  sprint_gauge_info_item(info+bytes, max-bytes,"gauge.fix.description","%s",
			 param.gauge_fix_description,0,0);
  bytes = strlen(info);
  sprint_gauge_info_item(info+bytes, max-bytes,"gauge.fix.tolerance","%e",
			 (char *)&param.gauge_fix_tol,0,0);
  bytes = strlen(info);
  sprint_gauge_info_item(info+bytes, max-bytes,"gauge.translate","%d",
			 (char *)&param.rshift,4,sizeof(int));
  bytes = strlen(info);
