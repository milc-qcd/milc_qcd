...
/*		    if(this_node==0)printf("color=%d spin=%d\n",color,spin); */
	  ...
	  /* spectrum */
	  node0_printf("BEGIN PROPAGATORS\n");
#ifdef CVC
    for(color=0;color<3;color++){
...
/* BEGIN ROTATED MESON OPERATORS   
	for(ci=0;ci<3;ci++){

	      for(spinindex=0;spinindex<n_spins;spinindex++){
		spin = spins[spinindex];
		rotate_field(F_OFFSET(quark_propagator.c[ci].d[spin]),
		F_OFFSET(extra_propagator.d[spin]),m0);
		}

		w_meson_p(F_OFFSET(extra_propagator),
		    F_OFFSET(extra_propagator), rmes_prop[k]);
	}
 END ROTATED MESON OPERATORS */


/* BEGIN SMEARED MESON PROPAGATOR */

	space_vol = (Real)(nx*ny*nz);
	inv_space_vol=1.0/space_vol;

	    /* Now convolute the quark propagator with a Gaussian for
	       the smeared mesons. This is done with FFT's, one
		color at a time */
...
/* first copy the quark prop into extra prop, FFT is done in place */

	FORALLSITES(i,s) s->extra_propagator=s->quark_propagator.c[ci];


	    /* fft extra_propagator (in place)--use mp as working space */
	      for(spinindex=0;spinindex<n_spins;spinindex++){
		spin = spins[spinindex];
	...
	spin=0;color=0;
		...
		CMUL(s->extra_propagator.d[si].d[sf].c[cf],
		    s->chi.d[spin].c[color],
		    s->extra_propagator.d[si].d[sf].c[cf]);
	    }

/* The following note applies only to k=0 mesons! */
...
/* instead, we fft the quark propagator back to x space, with a volume
factor */
	    /* fft extra_propagator (in place)--use mp as working space */
	      for(spinindex=0;spinindex<n_spins;spinindex++){
		spin = spins[spinindex];
		...
		FORALLSITES(i,s)for(si=0;si<4;si++)
                scalar_mult_wvec(&(s->extra_propagator.d[si]), inv_space_vol,
                    &(s->extra_propagator.d[si]));


		w_meson_p(F_OFFSET(extra_propagator),
		    F_OFFSET(extra_propagator), smes_prop[k]);
	    } /* ci */

	} /* masses */

/* END SMEARED MESON PROPAGATOR */
