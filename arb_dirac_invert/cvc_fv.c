...
/* conserved vector current and ``hyp'' vector current for hypercube */
/* source is  source gamma matrix, sink is current gamma matrix */
...
of offsets (yes, it is expensive) */
	...
	s->hvct[nu].imag=0.0;
        ...
        mult_by_gamma_l( &(antiquark), &(localmat), source);
	antiquark=localmat;
...
dirac matrices for the current operator */
		...
		for(mu=0;mu<4;mu++) n[mu] = -end_offset[mu];
                ...
                scalar_mult_add_wvec(&wvec4,&wvec1,pathsign*lambda[k],&wvec3);
	...
	/* trace over propagators to form the (offset) cvc */
	    for(sf=0;sf<4;sf++)
		  ...
		  g1 = wvec4.d[sf].c[cf];
		  s->hvct[nu].real += (g1.real*g2.real - g1.imag*g2.imag);
		  s->hvct[nu].imag += (g1.real*g2.imag + g1.real*g2.imag);
			}
	} /* si  */
    } /* sites */
...
mid_offset. We must gather up from below and add to current*/
		...
		sizeof(complex),
                ...
                tag[0]=start_general_gather_site(F_OFFSET(hvct[nu]),
		sizeof(complex),
                n_offset, EVENANDODD, gen_pt[0] );

                wait_general_gather(tag[0]);
                FORALLSITES(i,s){CSUM(s->hvc[nu],  *(complex *)(gen_pt[0][i]));}
                cleanup_general_gather(tag[0]);
...
/* print out time averaged observable */
    ...
    FORALLSITES(i,s) {
        my_t = s->t;
                ...
                cvc_prop[j][my_t] += (s->cvc[nu].real)*
                                (cx+cy+cz)/3.0;
                hvc_prop[j][my_t] += (s->hvc[nu].real)*
                                (cx+cy+cz)/3.0;
                }
                ...
                cvc_prop[3][my_t] += (s->cvc[nu].real)*
                                (cxy+cyz+cxz)/3.0;
                cvc_prop[4][my_t] += (s->cvc[nu].real)*
                                c111;

                hvc_prop[3][my_t] += (s->hvc[nu].real)*
                                (cxy+cyz+cxz)/3.0;
                hvc_prop[4][my_t] += (s->hvc[nu].real)*
                                c111;
