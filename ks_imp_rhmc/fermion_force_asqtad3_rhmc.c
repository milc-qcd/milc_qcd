...
void side_link_3f_force(int mu, int nu, Real coeff[2], 
	...
	half_wilson_vector *Path_mu, half_wilson_vector *Path_numu) ;
...
while( j<= stoporder-VECLENGTH+1 ){
  int jj;
  eo_fermion_force_rhmc_int( eps, &(residues[j]), multi_x+j-1, VECLENGTH ); //TRY_ONE
...
//eo_fermion_force_rhmc_oneterm( eps, residues[j], F_OFFSET(xxx1) ); //TRY_TWO
//} //TRY_TWO
...
for( ;j<=stoporder-1;j+=2){ //TRY_ONE_OR_TWO
	...
	eo_fermion_force_rhmc_twoterms( eps, residues[j], residues[j+1],
	    F_OFFSET(xxx1), F_OFFSET(xxx2) );
    }
    //for(j=1;j<=order-1;j+=2){
        //dslash_field( multi_x[j-1], multi_x[j-1],  ODD );
        //dslash_field( multi_x[j  ], multi_x[j  ], ODD );
	//eo_fermion_force_rhmc_int( eps, &(residues[j]), multi_x+j-1, TWO );
    //}
    //TMPfor( ; j<=order; j++ ){
for( ; j<=stoporder; j++ ){
	FOREVENSITES(i,s){ s->xxx1 = multi_x[j-1][i] ; }
        dslash_site( F_OFFSET(xxx1), F_OFFSET(xxx1), ODD );
	eo_fermion_force_rhmc_oneterm( eps, residues[j], F_OFFSET(xxx1) );
    }
...
/**********************************************************************/
/*   Version for two terms in the action		             */
/**********************************************************************/
...
//#ifdef NOTDEFINED
void eo_fermion_force_rhmc_twoterms( Real eps, Real res1, Real res2,
...
void eo_fermion_force_rhmc_int( Real eps, Real *residues, su3_vector **xxx, int nterms ) {
  ...
  for(mu = 0; mu < 8; mu++){
    ...
    if(pt == NULL){
      printf("eo_fermion_force_rhmc: No room for pt\n");
      terminate(1);
    }
    vl[mu] = pt;
  }
  ...
  if(Pmu == NULL){
    printf("eo_fermion_force_rhmc: No room for Pmu\n");
    terminate(1);
  }
  
  Pmumu = (veclist *)special_alloc(sites_on_node*sizeof(veclist));
  if(Pmumu == NULL){
    printf("eo_fermion_force_rhmc: No room for Pmumu\n");
    terminate(1);
  }
  ...
  for(mu=0; mu<8; mu++) {
    ...
    if(P3[mu] == NULL){
      printf("eo_fermion_force_rhmc: No room for P3\n");
      terminate(1);
    }
    P5[mu]= (veclist *)malloc(sites_on_node*sizeof(veclist));
    if(P5[mu] == NULL){
      printf("eo_fermion_force_rhmc: No room for P5\n");
      terminate(1);
    }
  }
  ...
  FORALLUPDIR(dir){
    ...
    pt = (su3_matrix *)malloc(sites_on_node*sizeof(su3_matrix));
    if(pt == NULL){
      printf("eo_fermion_force_rhmc: No room for backwardlink\n");
      terminate(1);
    }
    backwardlink[dir] = pt;
  }
  ...
  FORALLUPDIR(dir){
    ...
    pt = (anti_hermitmat *)malloc(sites_on_node*sizeof(anti_hermitmat));
    if(pt == NULL){
      printf("eo_fermion_force_rhmc: No room for tempmom\n");
      terminate(1);
    }
    tempmom[dir] = pt;
  }
  ...
  for(mu = 0; mu < 8; mu++){
    ...
    if(temp_vl[mu] == NULL){
      printf("eo_fermion_force_rhmc: No room for temp_vl\n");
      terminate(1);
    }
  }
...
void u_shift_hw_fermion(half_wilson_vector *src, half_wilson_vector *dest, 
  ...
  if(GOES_FORWARDS(dir)) /* forward shift */
	...
	mult_su3_mat_hwvec( &(s->link[dir]), 
			    ...
			    dest + i );
#else
      mult_su3_sitelink_lathwvec(dir, (half_wilson_vector **)gen_pt[dir], dest);
#endif
    }
  else /* backward shift */
    {
	...
	mult_adj_su3_mat_hwvec( backwardlink[OPP_DIR(dir)] + i, 
      ...
      mult_adj_su3_fieldlink_lathwvec( backwardlink[OPP_DIR(dir)],
	       (half_wilson_vector **)gen_pt[dir], dest);
#endif
    }
