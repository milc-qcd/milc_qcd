...
/* Modifications
   ...
   2/14/98 Added start/restart_gather_from_temp:
             neighbor[] is now list of indices;
           added routine g_xor32 U.M.H
...
int n_send_msgs, n_recv_msgs;
	...
	pvm_send( mbuf[i].msg_node, GATHER_MSG_TYPE(mynode(),type_offset) );
    }
    /* terminate list */
    if(mbuf != NULL){
...
int n_send_msgs, n_recv_msgs;
	...
	/* start the send */
	pvm_pkbyte( (char *)mbuf[i].msg_buf, mbuf[i].msg_size, 1 );
	pvm_send( compt->othernode, GATHER_MSG_TYPE(mynode(),type_offset));
    }
...
void g_wvectosumReal( g_wvectosumReal *wvpt) {
