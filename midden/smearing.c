...
Real conver;
	...
	FORALLSITES(i,s){
	    scalar_mult_su3_matrix( &(s->link[dir]), smear_fac, &(s->staple));
	}

	/* Loop over other directions, computing contribution from
	   plaquettes in the (dir,diro)-plane */
	for(diro=XUP;diro<=ZUP;diro++) if(diro != dir){
	    ...
	    tag2 = start_gather( F_OFFSET(diag), sizeof(su3_matrix),
		...
		add_su3_matrix( &(s->staple), &tmat2, &(s->staple));
	    }
	    cleanup_gather(tag0);
	    cleanup_gather(tag1);
		...
		add_su3_matrix( &(s->staple), &tmat1, &(s->staple));
	    }
	    cleanup_gather(tag2);
	...
	/* Start with the "old" link and put result into diag */
	FORALLSITES(i,s){
	    su3mat_copy( &(s->link[dir]), &(s->diag) );
	}

	old_tr = (double)0.0;
	FORALLSITES(i,s){
	    old_tr += (double)realtrace_su3(&(s->diag), &(s->staple) );
	}
	...
	for(count=0; count<MAXCOUNT && conver>TOL; count++){
		...
		FORALLSITES(i,s){
	            mult_su3_na( &(s->diag), &(s->staple), &tmat1 );
		    ...
		    /* Do the SU(2) hit */
		    left_su2_hit_n( &u, ind1, ind2, &(s->diag));
	    ...
	    new_tr = (double)0.0;
	    FORALLSITES(i,s){
		new_tr += (double)realtrace_su3(&(s->diag), &(s->staple) );
	    }
