...
*/
#define mat_invert mat_invert_uml2
...
#define SMEAR 2
  ...
  if(SMEAR>=1)smear_links( F_OFFSET(link[0]), F_OFFSET(smearlink[0]), staple_weight );
  else FORALLSITES(i,s)for(dir=XUP;dir<=TUP;dir++){
    s->smearlink[dir] = s->link[dir];
  }
  for(j=2;j<=SMEAR;j++){
    FORALLSITES(i,s)for(dir=XUP;dir<=TUP;dir++){
      s->field_strength[dir] = s->smearlink[dir];
    }
    smear_links( F_OFFSET(field_strength[0]), F_OFFSET(smearlink[0]), staple_weight );
  }
...
if(s->parity==EVEN)
	...
	/* TEMP: test inversion, */
	check_invert2( F_OFFSET(quark_prop), F_OFFSET(quark_source) );
...
/* WE NEED TO NORMALIZE THESE ROUTINES WITH generic_ks/mat_invert.c -CD!!*/
...
int mat_invert_cg2( field_offset src, field_offset dest ){
    int cgn;
    Real finalrsq;
    if( src != F_OFFSET(phi) )copy_latvec( src, F_OFFSET(phi), EVENANDODD );
    ...
    scalar_mult_add_latvec( F_OFFSET(ttt), F_OFFSET(xxx),
...
int mat_invert_uml2(field_offset src, field_offset dest ){
    ...
    scalar_mult_add_latvec( F_OFFSET(ttt), src,
       -2.0*mass, F_OFFSET(phi), EVEN);
    scalar_mult_latvec( F_OFFSET(phi), -1.0, F_OFFSET(phi), EVEN);
    /* invert with M_adj M even */
    cgn = ks_congrad(F_OFFSET(phi),F_OFFSET(xxx),mass,
				   niter,rsqprop,EVEN,&finalrsq);
    /* multiply by (1/2m)L, does nothing to even sites */
    copy_latvec( F_OFFSET(xxx), dest, EVEN);
    /* fix up odd sites , 1/2m (Dslash_oe*xxx_e + phi_odd) */
    dslash( F_OFFSET(xxx), F_OFFSET(ttt), ODD);
    FORODDSITES(i,s){
	sub_su3_vector( (su3_vector *)F_PT(s,src), &(s->ttt), &(s->xxx) );
	scalar_mult_su3_vector( &(s->xxx), 1.0/(2.0*mass),
	    (su3_vector *)F_PT(s,dest) );
    }
    return(cgn);
}
...
void check_invert2( field_offset src, field_offset dest ){
    ...
    FORALLSITES(i,s){
	    ...
	    if( fabs( ((su3_vector *)F_PT(s,dest))->c[k].real
		- s->cg_p.c[k].real) > 2e-4 )flag=1;
	    if( fabs( ((su3_vector *)F_PT(s,dest))->c[k].imag
		- s->cg_p.c[k].imag) > 2e-4 )flag=1;
...
/* DOUG: In control.c smear_links is called with phases off. C.D. */
