...
/* Modifications */
/* 4/17/98 r_parallel: g_syncs to prevent shmem message pileups C.D. */
...
/* Checksums
   ...
   files appear as a series of 32-bit Realing point numbers.  We treat
...
void w_serial(gauge_file *gf)
{
  ...
  if(this_node==0)
	...
	{
	  printf("w_serial: Node %d fseeko %ld failed error %d file %s\n",
  ...
  for(j=0,t=0;t<nt;t++)for(z=0;z<nz;z++)for(y=0;y<ny;y++)for(x=0;x<nx;x++,j++)
      ...
      if(this_node==0)
	{
	  ...
	  for(k = 0, val = (u_type32 *)&lbuf[4*buf_length]; 
	      k < 4*(int)sizeof(su3_matrix)/(int)sizeof(type32); k++, val++)
	    {
...
void read_site_list(int parallel,gauge_file *gf)
{
  ...
  if(gf->header->order != NATURAL_ORDER)
      ...
      if(this_node==0)
	{
	  ...
	  if(parallel)
	    {
	      if((int)g_read(gf->rank2rcv,sizeof(type32),volume,gf->fp) != volume )
		{
	    ...
	    {
	      if((int)fread(gf->rank2rcv,sizeof(type32),volume,gf->fp) != volume )
		{
		  printf("read_site_list: Node %d site list read error %d\n",
			 this_node,errno);
		  terminate(1);	
...
void r_serial(gauge_file *gf)
{
  ...
  if(this_node == 0)
      ...
      if(lbuf == NULL)
	{
      ...
      if( fseeko(fp,offset,SEEK_SET) < 0 ) 
	{
	  printf("%s: Node 0 fseeko %ld failed error %d file %s\n",
  ...
  for(rcv_rank=0; rcv_rank<volume; rcv_rank++)
      ...
      if(this_node==destnode)
	{
	  ...
	  for(k = 0, val = (u_type32 *)&lattice[i].link[0]; 
	      k < 4*(int)sizeof(su3_matrix)/(int)sizeof(type32); k++, val++)
	    {
	    ...
	    isite<sites_on_node && isite<ksite+site_block; isite++)
	  {
		...
		rank29 = rank31 = 4*sizeof(su3_matrix)/sizeof(type32)*i;
		 ...
		 are inserted into its buffer */
	      rank29 %= 29; rank31 %= 31;
	      for(k = 0, val = (u_type32 *)&lbuf[4*where_in_buf]; 
		  k < 4*(int)sizeof(su3_matrix)/(int)sizeof(type32); k++, val++)
		{
		  gf->check.sum29 ^= (*val)<<rank29 | (*val)>>(32-rank29);
		  gf->check.sum31 ^= (*val)<<rank31 | (*val)>>(32-rank31);
