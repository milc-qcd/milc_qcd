...
void w_serial_w(w_prop_file *wpf, int spin, int color, field_offset src)
{
  ...
  rank29 = sizeof(wilson_vector)/sizeof(type32)*sites_on_node*this_node % 29;
  rank31 = sizeof(wilson_vector)/sizeof(type32)*sites_on_node*this_node % 31;
  ...
  for(j=0,t=0;t<nt;t++)for(z=0;z<nz;z++)for(y=0;y<ny;y++)for(x=0;x<nx;x++,j++)
      ...
      if(this_node==0)
	{
	  ...
	  for(k = 0, val = (type32 *)&lbuf[buf_length]; 
	      k < (int)sizeof(wilson_vector)/(int)sizeof(type32); k++, val++)
	    {
...
/*      printf("Wrote prop serially for spin %d color %d to file %s\n",
	...
	{
	  printf("w_serial_w: Node %d fseeko %ld failed error %d file %s\n",
		 this_node,(long)offset,errno,wpf->filename);
	  fflush(stdout);terminate(1);   
	}
...
void read_site_list_w(int parallel, w_prop_file *wpf)
{
  ...
  if(wpf->header->order != NATURAL_ORDER)
    {
      wpf->rank2rcv = (type32 *)malloc(volume*sizeof(type32));
      ...
      if(this_node==0)
	{
	  ...
	  /* Reads receiving site coordinate if file is not in natural order */
          if(parallel)
            {
	      if((int)g_read(wpf->rank2rcv,sizeof(type32),volume,wpf->fp) 
		 != volume )
		{
 	    ...
 	    {
	      if((int)fread(wpf->rank2rcv,sizeof(type32),volume,wpf->fp) 
		 != volume )
		{
		  printf("read_site_list_w: Node %d site list read error %d\n",
			 this_node,errno);
      ...
      broadcast_bytes((char *)wpf->rank2rcv,volume*sizeof(type32));
    }
...
int read_1996_w_prop_hdr(w_prop_file *wpf, int parallel, int *byterevflag)
{
  ...
  if(wph->magic_number == W_PROP_VERSION_NUMBER_1996) 
    ...
    {
      byterevn((type32 *)&wph->magic_number,1);
      if(wph->magic_number == W_PROP_VERSION_NUMBER_1996) 
	{
	  *byterevflag=1;
...
int r_serial_w(w_prop_file *wpf, int spin, int color, field_offset src)
{
  ...
  if(this_node == 0)
      ...
      if(lbuf == NULL)
	{
      ...
      if( fseeko(fp,offset,SEEK_SET) < 0 ) 
	{
	  printf("%s: Node %d fseeko %ld failed error %d file %s\n",
  ...
  for(rcv_rank=0; rcv_rank<volume; rcv_rank++)
	...
	{
	  if(byterevflag==1)
	    byterevn((type32 *)dest,
		     sizeof(wilson_vector)/sizeof(type32));
	  /* Accumulate checksums */
	  for(k = 0, val = (type32 *)dest; 
	      k < (int)sizeof(wilson_vector)/(int)sizeof(type32); k++, val++)
	    {
	...
	{
	  rank29 += sizeof(wilson_vector)/sizeof(type32);
	  rank31 += sizeof(wilson_vector)/sizeof(type32);
	  rank29 %= 29;
	  rank31 %= 31;
	}
    }
...
void write_site_list_w(FILE *fp, w_prop_header *wph)
{
  ...
  if( (int)g_write(cbuf,sizeof(type32),sites_on_node,fp) != sites_on_node)
    {
  ...
  for(ksite=0; ksite<sites_on_node; ksite += site_block)
	...
	for(isite=ksite; 
	    isite<sites_on_node && isite<ksite+site_block; isite++)
	  {
		...
		rank29 = rank31 = 
		  sizeof(wilson_vector)/sizeof(type32)*rcv_rank;
	      }
	      else {
		/* Receive a message */
		...
		rank29 = rank31 = sizeof(wilson_vector)/sizeof(type32)*i;
	      }

	      /* Receiving node accumulates checksums as the values
		 are inserted into its buffer */
	      rank29 %= 29; rank31 %= 31;
	      for(k = 0, val = (type32 *)&lbuf[where_in_buf]; 
		  k < (int)sizeof(wilson_vector)/(int)sizeof(type32); 
		  k++, val++)
		{
		  wpf->check.sum29 ^= (*val)<<rank29 | (*val)>>(32-rank29);
		  wpf->check.sum31 ^= (*val)<<rank31 | (*val)>>(32-rank31);
  ...
  for(ksite=0; ksite<sites_on_node; ksite += site_block)
	...
	for(isite=ksite; 
	    isite<sites_on_node && isite<ksite+site_block; isite++)
	  {
		...
		byterevn((type32 *)&lbuf[where_in_buf],
			 sizeof(wilson_vector)/sizeof(type32));
  ...
  for(t=0;t<nt;t++)for(z=0;z<nz;z++)for(y=0;y<ny;y++)for(x=0;x<nx;x++)
      ...
      if(this_node==0)
	{
	  if(currentnode==0)
	    ...
	    {
	      get_field((char *)&lbuf,sizeof(wilson_vector));
	    }
...
w_prop_file *r_ascii_w_i(char *filename)
{
  ...
  if(this_node==0)
      ...
      if( (fscanf(fp,"%d",&wph->magic_number))!=1 )
	{
      ...
      if(wph->magic_number != W_PROP_VERSION_NUMBER)
	{
	  printf("r_ascii_w_i: Unrecognized magic number in gauge configuration file header.\n");
