...
#define RHO_SOURCE
#define A1P_SOURCE
#define ZEROPM_SOURCE
#define ZEROPMP_SOURCE
#define ZEROPMB_SOURCE
#define ZEROMM_SOURCE
#define ZEROMMP_SOURCE
#define ONEMP_SOURCE
...
int spectrum_hybrids(){ /* return the C.G. iteration number */
  ...
  if(SMEAR>=1)smear_links( F_OFFSET(link[0]), F_OFFSET(smearlink[0]) );
  ...
  for(src_count=0,t_source=source_start; t_source<nt && src_count<n_sources;
	...
	/* TEMP: test inversion, */
	/**check_invert( F_OFFSET(quark_prop), F_OFFSET(quark_source) );**/
...
/* Compute M^-1 * src, answer in dest
  Uses chi, ttt, r, psi, mp and p as workspace */
#ifdef NONSENSE
int mat_invert_cg( field_offset src, field_offset dest ){
    ...
    dslash( F_OFFSET(chi), F_OFFSET(psi), PLUS, EVEN);
    FOREVENSITES(i,s){
    ...
    FORODDSITES(i,s){
        scalar_mult_add_wvec( &(s->psi), &(s->chi), kappa,
	    (wilson_vector *)F_PT(s,dest) );
    }
    FOREVENSITES(i,s){
	*((wilson_vector *)F_PT(s,dest)) = s->psi;
    }
    return(iters);
}
#endif
...
/* Compute M^-1 * src, answer in dest
  Uses chi, mp and p as workspace */
int mat_invert_cgilu( field_offset src, field_offset dest ){
    ...
    iters=cgilu_w( F_OFFSET(chi),dest,&qic,(void *)&dwp);
	...
	else{ printf("Can't restart inversion\n"); terminate(0);}
        iters+=cgilu_w( F_OFFSET(chi),dest,&qic,(void *)&dwp);
	if(iters==5*niter){if(this_node==0)
	    printf("CGILU: NO CONVERGE\n");
	    terminate(0);
	}
    }
/**if(this_node==0)printf("mat_invert_cgilu(): iters = %d\n",iters);**/
    return(iters);
}
...
int mat_invert_bicgilu( field_offset src, field_offset dest ){
    ...
    qic.start_flag = 0;
    qic.wv2 = F_OFFSET(mp);
    qic.wv3 = F_OFFSET(vtmp);  /* Called rv in bicg */
    qic.wv4 = F_OFFSET(sss);
    
    /* Load Dirac matrix parameters */
    dwp.Kappa = kappa;
    
    iters=bicgilu_w( F_OFFSET(chi),dest,&qic,(void *)&dwp);
    while(iters%niter==0){
      qic.start_flag = 1;
        if( src != F_OFFSET(chi) ){
	    FORALLSITES(i,s) s->chi = *(wilson_vector *)F_PT(s,src);
...
/**if(this_node==0)printf("mat_invert_bicgilu(): iters = %d\n",iters);**/
    return(iters);
}
#endif /* BICONGRAD */

int mat_invert_mrilu( field_offset src, field_offset dest ){
    register int i,j;
    register site *s;
    int iters;
    int MinCG = nt/2;
    ...
    iters=mrilu_w_or( F_OFFSET(chi),dest,&qic,(void *)&dwp);
    while(iters%niter==0){
      qic.start_flag = 1;
        if( src != F_OFFSET(chi) ){
	    FORALLSITES(i,s) s->chi = *(wilson_vector *)F_PT(s,src);
        ...
        iters+=mrilu_w_or( F_OFFSET(chi),dest,&qic,(void *)&dwp);
	if(iters==5*niter){if(this_node==0)
	    printf("MRILU: NO CONVERGE\n");
	    terminate(0);
	}
    }
/**if(this_node==0)printf("mat_invert_mrilu(): iters = %d\n",iters);**/
    return(iters);
}
...
s = &(lattice[1]); 
    ...
    gaugefix(XUP,(Real)1.8,500,(Real)GAUGE_FIX_TOL,0,NULL,NULL,0,NULL,NULL);
