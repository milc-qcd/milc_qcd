...
int v1, int v2) 
{
  ...
  if( tend > tstart )
  {
    printf("ERROR: generate_hqet_prop_back: tend %d > tstart %d \n",tend,tstart) ;
    terminate(1); 
  }
  ...
  {
    printf("ERROR: generate_hqet_prop_back: has nor been checked for v1!=v2");
    printf("ERROR: generate_hqet_prop_back: has nor been checked for v1!=v2");
    terminate(1) ;
  }
  ...
  for(tloop=tstart ; tloop >= tend ; --tloop)
  {
     ...
     *   W(t-1) =  U_4(t-1)    W(t)
     *
     *  Evolve the HQET propagator backwards in time
     *
     *
     */
    ...
    FORALLSITES(i,s)
    {
     if( s-> t == tloop )  
     {
       mult_su3_nn(&(s->link[TUP]), (su3_matrix *) gen_pt[TUP ][i], (su3_matrix *)   F_PT(s,hqet_prop)  );
     }
