...
void clear_links ( su3_matrix **link, size_t nlink ) {
	...
	if ( link == NULL ) {
		node0_printf("Tried to clear unallocated link\n"); 
		terminate(1);
	}
	if ( link[0] == NULL ) {
		node0_printf("Tried to clear unallocated link\n"); 
	...
	memset( link[0], '\0', nlink * sizeof(su3_matrix) * sites_on_node );
}
...
void destroy_links ( su3_matrix **link ) {
	...
	if ( link[0] == NULL ) {
		node0_printf("Tried to free unallocated link\n"); 
		terminate(1);
	}
...
su3_matrix ** new_links( int region_flag, size_t nlink ) {
	...
	register site *s;
	if( nlink > 4 ) {
		printf( "new_links: can't create than four links from site; instead %ld\n", nlink );
		fflush(stdout); terminate(1);
	}
	su3_matrix **this = (su3_matrix **)malloc( nlink * sizeof(su3_matrix*) );
	if(this == NULL) {
		printf( "new_links: can't malloc this\n" );
		fflush(stdout); terminate(1);
	}

	this[0] = (su3_matrix *)malloc( nlink * sizeof(su3_matrix) * sites_on_node );
	if(this[0] == NULL) {
		printf( "new_links: can't malloc this[0]\n" );
		fflush(stdout); terminate(1);
	}
	memset( this[0], '\0', nlink * sizeof(su3_matrix) * sites_on_node );
	for ( i = 1; i < nlink; i++ ) 
...
su3_matrix ** new_half_links( void ) {
	...
	int i;
	su3_matrix **this = (su3_matrix **)malloc( N_HALF * sizeof(su3_matrix*) );
	if(this == NULL) {
		printf( "new_half_links: can't malloc this\n" );
		fflush(stdout); terminate(1);
	}

	this[0] = (su3_matrix *)malloc( N_HALF * sizeof(su3_matrix) * sites_on_node );
	if(this[0] == NULL) {
		printf( "new_half_links: can't malloc this[0]\n" );
		fflush(stdout); terminate(1);
	}
	memset( this[0], '\0', N_HALF * sizeof(su3_matrix) * sites_on_node );
	for ( i = 1; i < N_HALF; i++ ) 
		this[i] = this[0] + i * sites_on_node;
...
void clear_last_flow_links ( su3_matrix ***link_last_flow ) {
	...
	if ( link_last_flow == NULL ) {
		node0_printf("Tried to clear unallocated link_last_flow\n"); 
		terminate(1);
	}
	...
	if ( link_last_flow[0][0] == NULL ) {
		node0_printf("Tried to clear unallocated link_last_flow\n"); 
		terminate(1);
	}

	memset( link_last_flow[0][0], '\0', 2 * N_LAST_FLOW * sizeof(su3_matrix) * sites_on_node );
}
...
void destroy_last_flow_links ( su3_matrix ***link_last_flow ) {
	...
	free( link_last_flow[0][0] );
	free( link_last_flow[0] );
	free( link_last_flow );
	link_last_flow = NULL;
}
...
su3_matrix *** new_last_flow_links( void ) {
	...
	int i;
	su3_matrix ***this = (su3_matrix ***)malloc( 2 * sizeof(su3_matrix**) );
	if(this == NULL) {
		printf( "new_last_flow_links: can't malloc this\n" );
		fflush(stdout); terminate(1);
	}

	for ( i=0; i <2; i++) {
		this[i] = (su3_matrix **)malloc( N_LAST_FLOW * sizeof(su3_matrix*) );
		if(this[i] == NULL) {
			printf( "new_last_flow_links: can't malloc this\n" );
			fflush(stdout); terminate(1);
		}
	...
	this[0][0] = (su3_matrix *)malloc( 2 * N_LAST_FLOW * sizeof(su3_matrix) * sites_on_node );
	if(this[0] == NULL) {
		printf( "new_half_links: can't malloc this[0]\n" );
		fflush(stdout); terminate(1);
	}
	memset( this[0][0], '\0', 2 * N_LAST_FLOW * sizeof(su3_matrix) * sites_on_node );
	for ( i = 1; i < 2 * N_LAST_FLOW; i++ ) 
		this[i / N_LAST_FLOW][i % N_LAST_FLOW] = this[0][0] + i * sites_on_node;
...
void update_last_flow_links( su3_matrix ***link_last_flow ) {
	...
	memcpy( link_last_flow[1][0], link_last_flow[0][0], (size_t)(N_LAST_FLOW * sizeof( su3_matrix ) * sites_on_node ) );
	FORALLUPDIRBUT(TUP,dir) 
		FORALLSITES(i, s) 
		IF_BLOCKED(s, block_stride)
		IF_BOUNDARY(s)
			su3mat_copy( &(s->link[dir]), &(link_last_flow[0][dir][i]) );
}
...
void report_bulk( Real time ) {
  ...
  gauge_action_w_s_full( &Et_W, &Es_W, &Et_S, &Es_S );

#if (MILC_PRECISION==1)
  node0_printf("FULL:  %g %g %g %g %g %g %g %g\n", time, Et_C, Es_C, Et_W, Es_W, Et_S, Es_S, charge);
#else
  node0_printf("FULL:  %g %.16g %.16g %.16g %.16g %.16g %.16g %.16g\n", time, Et_C, Es_C, Et_W, Es_W, Et_S, Es_S, charge);
#endif
...
#if (MILC_PRECISION==1)
  node0_printf("BULK:  %g %g %g %g %g %g %g %g\n", time, Et_C, Es_C, Et_W, Es_W, Et_S, Es_S, charge);
#else
  node0_printf("BULK:  %g %.16g %.16g %.16g %.16g %.16g %.16g %.16g\n", time, Et_C, Es_C, Et_W, Es_W, Et_S, Es_S, charge);
#endif
  fflush(stdout);

  fmunu_fmunu_half(&Et_C, &Es_C, &charge);
  gauge_action_w_s_half( &Et_W, &Es_W, &Et_S, &Es_S );

#if (MILC_PRECISION==1)
  node0_printf("HALF:  %g %g %g %g %g %g %g %g\n", time, Et_C, Es_C, Et_W, Es_W, Et_S, Es_S, charge);
#else
  node0_printf("HALF:  %g %.16g %.16g %.16g %.16g %.16g %.16g %.16g\n", time, Et_C, Es_C, Et_W, Es_W, Et_S, Es_S, charge);
#endif
  fflush(stdout);
...
// #if (MILC_PRECISION==1)
//   node0_printf("BDRY:  %g %g %g %g %g %g %g %g\n", stoptime, Et_C, Es_C, Et_W, Es_W, Et_S, Es_S, charge);
// #else
//   node0_printf("BDRY:  %g %.16g %.16g %.16g %.16g %.16g %.16g %.16g\n", stoptime, Et_C, Es_C, Et_W, Es_W, Et_S, Es_S, charge);
// #endif
//   fflush(stdout);
    ...
    for( dir[1]=XUP; dir[1]<dir[0]; dir[1]++ ) {
      ...
      #define LINK1 link[dir[1]]
 		  ...
 		  for (ig = 0; ig < NGATHER; ig++) {
			...
			ig = 0;
      // request link[dir[1]] from direction dir[0], not blocked in time direction
      tag[ig] = start_gather_site( F_OFFSET(link[dir[1]]) , sizeof(su3_matrix),
             dir[0], EVENANDODD, gen_pt[ig]);
      ...
      if ( block_stride > 2 ) 
      	...
      	IF_BLOCKED(s, block_stride)
      	IF_ACTIVE(s) 
      		su3mat_copy( &(s->link[dir[0]]) , &(temp[icur[ig]][i]) );

      // request link[dir[0]] from direction dirb[1]
      for (stride = block_stride; stride >= 1; stride -= 2, icur[ig] = ncur[ig]-icur[ig] ) {
      	if ( block_stride <=2 ) {
      		tag[ig] = start_gather_site( F_OFFSET(link[dir[0]]) , sizeof(su3_matrix),
        		     dirb[1], EVENANDODD, gen_pt[ig]);
      	} else {
	 				if ( stride > 2 ) { 
						btag = start_gather_field( temp[icur[ig]] , sizeof(su3_matrix),
																				dirb[1], EVENANDODD, gen_pt[ig]);
						wait_gather( btag );
						FORALLSITES(i,s) 
						IF_BLOCKED(s, block_stride)	
						IF_ACTIVE(s) 
							su3mat_copy( (su3_matrix*)(gen_pt[ig][i]), &(temp[ncur[ig]-icur[ig]][i]) );
						cleanup_gather( btag );
					} else {
						tag[ig] = start_gather_field( temp[icur[ig]] , sizeof(su3_matrix),
        		     dirb[1], EVENANDODD, gen_pt[ig]);
      		} 
				}
			}
      ...
      IF_BLOCKED(s, block_stride)	
 			IF_LOWER_BULK(s) {
				// su3_adjoint( &(s->link[dir[1]]), &(su3mat[0][i]) );
        mult_su3_nn( &(su3mat[0][i]), (su3_matrix *)(gen_pt[0][i]), &(su3mat[3][i]) );
      }
      ...
      IF_BLOCKED(s, block_stride) 
        ...
        // form a staple for "top" link in the plaquette
        mult_su3_an( (su3_matrix *)(gen_pt[1][i]), &(su3mat[0][i]), &(su3mat[2][i]) );
        // su3_adjoint( &(s->link[dir[1]]), &(su3mat[2][i]) );
			}
      ...
      // // request link[dir[0]] from direction dir[1]
      // for (stride = block_stride; stride >= 1; stride -= 2, icur[ig] = ncur[ig]-icur[ig] ) {
      // 	if ( block_stride <=2 ) {
      // 		tag[ig] = start_gather_site( F_OFFSET(link[dir[0]]) , sizeof(su3_matrix),
      //   		     dirb[1], EVENANDODD, gen_pt[ig]);
      ...
      for (stride = block_stride; stride >= 1; stride -= 2, icur[ig] = ncur[ig]-icur[ig] ) {
      	if ( block_stride <=2 ) {
			...
			//fflush(stdout);printf("dir[0]=%d dir[1]=%d\n",dir[0],dir[1]);fflush(stdout);
      // form a staple for "bottom" link in the plaquette
      FORALLSITES(i, s) 
      IF_BLOCKED(s, block_stride) 
 			// must include here one time step more, since su3mat[4] is gathered down one time step
