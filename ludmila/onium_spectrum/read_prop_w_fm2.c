...
void r_sc_prop_w_fm(char *filename)
{
  ...
  if(this_node==0){
	...
	elements_per_site != 24 /* wilson_propagator */)
      {	
  ...
  for(t=0;t<nt;t++)for(z=0;z<nz;z++)for(y=0;y<ny;y++)for(x=0;x<nx;x++){
    ...
    if(this_node==0){
      if(psread_byteorder(byterevflag,0,fp,&q,sizeof(q),
			  filename,"reading the wilson propagator")!=0) terminate(1);
	...
	i = node_index(x,y,z,t);
	  for(s1=0;s1<4;s1++)for(c1=0;c1<3;c1++)
	    {
	      lattice[i].quark_propagator.c[c0].d[s0].d[s1].c[c1].real 
		= q.d[s1].c[c1].real;
	      lattice[i].quark_propagator.c[c0].d[s0].d[s1].c[c1].imag 
		= q.d[s1].c[c1].imag;
	    }
      }
      else {		/* send to correct node */
	send_field((char *)&q, sizeof(q),destnode);
      }
