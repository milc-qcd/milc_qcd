...
#define TSLICE 4  /* width of timeslice */
...
void instanton_density(void)
{
   ...
   if( (partsum = malloc(nparts*sizeof(Real))) == NULL ){
     printf("node %d can't malloc partsum for %d Reals\n",nparts);
     terminate(1);
   }
   ...
   FORALLSITES(i,s)
   {
      s->ch_dens *= NORM;
      sum += s->ch_dens;
      partsum[s->t/TSLICE] += s->ch_dens;
   }

   g_floatsum( &sum );
   for(i = 0; i < nparts; i++){
     g_floatsum( &partsum[i] );
   }
