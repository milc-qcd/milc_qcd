			...
			, fn_links_t *fn)			 
{
  ...
  return iterations_used;
    ...
    int iters;
    int otherparity;
    struct QuarkInvertControl grid_qic;
...
#endif   
    /* Pack structure */
    grid_qic.prec      = prec;       /* Currently ignored */
    grid_qic.parity    = parity;
    grid_qic.max       = niter;
    grid_qic.nrestart  = nrestart;
    grid_qic.resid     = rsqmin;
    grid_qic.relresid  = 0;          /* Suppresses this test */
...
#warning using unoptimized gathers
    ...
    t_sp1 = -dclock();
#endif    
    get_ks_spinors_from_lattice(src, g_grid_env_obj->ks_src1, parity);
#if CG_DEBUG
    t_sp1 += dclock();
    t_sp2 = -dclock();
#endif    
    get_ks_spinors_from_lattice(sol, g_grid_env_obj->ks_dest1, parity);
#if CG_DEBUG
    t_sp2 += dclock();
    t_ll1 = -dclock(); 
#endif     
    get_longlinks_from_lattice(g_grid_env_obj->gll[0], EVEN, fn);
#if CG_DEBUG
    t_ll1 += dclock(); 
    t_ll2 = -dclock();
#endif       
    get_longlinks_from_lattice(g_grid_env_obj->gll[1], ODD, fn);
#if CG_DEBUG
    t_ll2 += dclock();
    t_fl1 = -dclock(); 
#endif       
    get_fatlinks_from_lattice(g_grid_env_obj->gfl[0], EVEN, fn);
#if CG_DEBUG
    t_fl1 += dclock();
    t_fl2 = -dclock(); 
#endif       
    get_fatlinks_from_lattice(g_grid_env_obj->gfl[1], ODD, fn);
#if CG_DEBUG
    t_fl2 += dclock();
...
#warning using optimized gather
    ...
    t_sp1 = -dclock();
#endif    
    //gather_su3vectors_from_lattice(src_arg, g_grid_env_obj->ks_src1, parity);
    get_ks_spinors_from_lattice(src, g_grid_env_obj->ks_src1, parity);
#if CG_DEBUG
    t_sp1 += dclock();
    t_sp2 = -dclock();
#endif    
    get_ks_spinors_from_lattice(sol, g_grid_env_obj->ks_dest1, parity);
    //gather_su3vectors_from_lattice(dest_arg, g_grid_env_obj->ks_dest1, parity);
#if CG_DEBUG
    t_sp2 += dclock();
    t_ll = -dclock(); 
#endif         
    get_links_from_lattice(g_grid_env_obj->gfl, g_grid_env_obj->gll, fn);
#if CG_DEBUG
    t_ll += dclock();
    node0_printf("MILC-->Grid data layout conversion timings"
                 " (Optimized Gathers).\n"
    ...
    dtime = -dclock();
#endif    
    iters = grid_ks_congrad_parity ( g_grid_env_obj->ks_src1
                                    , g_grid_env_obj->ks_dest1
                                    , &grid_qic
                                    , mass
                                    , g_grid_env_obj->gll
                                    , g_grid_env_obj->gfl
    ...
    /* Unpack the results */
    qic->final_rsq    = grid_qic.final_rsq;
    
#ifdef CG_DEBUG
    ttime = -dclock();
#endif
    /* Copy results back to su3_vector */
    set_ks_spinors_into_lattice (sol, g_grid_env_obj->ks_dest1, parity);
