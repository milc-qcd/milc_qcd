/******* ks_multicg_offset_grid.c - multi-mass CG for SU3/fermions ****/
/* MIMD version 7 */
   ...
   )
{
  ...
  if(qic->prec == 1)
    iters = ks_multicg_offset_field_grid_F(src, psim, ksp, num_offsets, qic, fn);
  else
    iters = ks_multicg_offset_field_grid_D(src, psim, ksp, num_offsets, qic, fn);
