 ...
 * (algorithm previously found in quark_stuff4.c)
...
void side_link_3f_force_pppp(int mu, int nu, Real coeff[2], 
			half_wilson_vector **Path   , 
			half_wilson_vector **Path_nu, 
			half_wilson_vector **Path_mu, 
			half_wilson_vector **Path_numu) ;
void side_link_3f_force_npnp(int mu, int nu, Real coeff[2], 
			half_wilson_vector *Path   , 
			half_wilson_vector **Path_nu, 
			half_wilson_vector *Path_mu, 
			half_wilson_vector **Path_numu) ;
void side_link_3f_force_nnpp(int mu, int nu, Real coeff[2], 
			half_wilson_vector *Path   , 
			half_wilson_vector *Path_nu, 
			half_wilson_vector **Path_mu, 
			half_wilson_vector **Path_numu) ;
void side_link_3f_force_nnnp(int mu, int nu, Real coeff[2], 
			half_wilson_vector *Path   , 
			half_wilson_vector *Path_nu, 
			half_wilson_vector *Path_mu, 
			half_wilson_vector **Path_numu) ;
...
void eo_fermion_force_twoterms( Real eps, Real weight1, Real weight2,
  ...
  for(mu = 0; mu < 8; mu++){
    ...
    if(pt == NULL){
      printf("eo_fermion_force_twoterms: No room for hw_pt\n");
      terminate(1);
    }
    hw_pt[mu] = pt;
  }
  ...
  FORALLUPDIR(dir){
    ...
    if(pt == NULL){
      printf("eo_fermion_force_twoterms: No room for forwardlink\n");
      terminate(1);
    }
    forwardlink[dir] = pt;
  }
