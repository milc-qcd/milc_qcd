/**************** fermion_links_hypisq_load_milc.c *****************************/

/* CD. PLACEHOLDER COPIED FROM fermion_links_hisq_load_milc.c. NEEDS REVISION */
...
static void  
load_U_from_field(info_t *info, hypisq_auxiliary_t *aux, su3_matrix *links){
  //  int dir,i;
...
static void  
load_V_from_U(info_t *info, hypisq_auxiliary_t *aux, ks_component_paths *ap1){
  su3_matrix *U_link = aux->U_link;
...
static void 
load_Y_from_V(info_t *info, hypisq_auxiliary_t *aux, int umethod){
  int dir, i;
  su3_matrix *V_link = aux->V_link;
  ...
  INFO_HYPISQ_SVD_COUNTER(info) = svd_calls;
  return;
}
...
load_Y_from_V(info_t *info, hypisq_auxiliary_t *aux, int umethod){
...
#else  /* HYPISQ_REUNITARIZATION_DEBUG */
  ...
  case UNITARIZE_HYPISQ:
    node0_printf("UNITARIZE_HYPISQ: not ready!\n"); 
    terminate(0);
    break;
...
void 
load_W_from_Y(info_t *info, hypisq_auxiliary_t *aux, int umethod, int ugroup){
...
#endif
/*--------------------------------------------------------------------*/
/* Smear the W links to make the HYPISQ links.  Result in fn            */
/*--------------------------------------------------------------------*/

static void  
load_X_from_W(info_t *info, fn_links_t *fn, hypisq_auxiliary_t *aux,
...
static void 
load_hypisq_aux_links_cpu(info_t *info, ks_action_paths_hypisq *ap, hypisq_auxiliary_t *aux,
			su3_matrix *links){
  char myname[] = "load_hypisq_aux_links";
  double final_flop = 0.0;
...
static void 
load_hypisq_fn_links(info_t *info, fn_links_t **fn, fn_links_t *fn_deps,
		     hypisq_auxiliary_t *aux, ks_action_paths_hypisq *ap, 
		     su3_matrix *links, int want_deps, int want_back){

  //char myname[] = "load_hypisq_fn_links";
  int inaik;
...
//   printf("Enter look_at_link fermion_links_hypisq.c\n");
// #endif
...
//   dumpmat( &Xlongsum );
// #ifdef AB_DEBUG_ENTRY_EXIT_ROUTINES
//   printf("Exit  look_at_link fermion_links_hypisq.c\n");
// #endif
// }
...
/*-----------------------------------------------------*/
/* Create/destroy the hypisq_auxiliary_t structure      */
/*-----------------------------------------------------*/

/* Create all but the last asqtad smearing */
static hypisq_auxiliary_t *
create_hypisq_auxiliary_t(ks_action_paths_hypisq *ap, su3_matrix *links){

  hypisq_auxiliary_t *aux;
  char myname[] = "create_hypisq_auxiliary_t";
  ...
  aux = (hypisq_auxiliary_t *)malloc(sizeof(hypisq_auxiliary_t));
  if(aux == NULL){
    printf("%s: no room\n",myname);
    terminate(1);
  }
...
void
destroy_hypisq_auxiliary_t(hypisq_auxiliary_t *aux){
...
/*-------------------------------------------------------------------*/
/* Create/destroy HYPISQ fn links                                      */
/*-------------------------------------------------------------------*/

void 
create_hypisq_links_milc(info_t *info, fn_links_t **fn, fn_links_t **fn_deps,
			 hypisq_auxiliary_t **aux, ks_action_paths_hypisq *ap, 
			 su3_matrix *links, int want_deps, int want_back){
  //char myname[] = "create_hypisq_links_milc";
  ...
  *aux = create_hypisq_auxiliary_t(ap, links);
  
  load_hypisq_aux_links(info, ap, *aux, links);
  final_flop += info->final_flop;
  ...
  load_hypisq_fn_links(info, fn, *fn_deps, *aux, ap, links, 
		     want_deps, want_back);
  final_flop += info->final_flop;
...
void
destroy_hypisq_links_milc(ks_action_paths_hypisq *ap, hypisq_auxiliary_t *aux, 
			  fn_links_t **fn, fn_links_t *fn_deps){

  //char myname[] = "destroy_hypisq_links_milc";

  int n_naiks = ap->n_naiks;
  int i;

  destroy_hypisq_auxiliary_t(aux);
