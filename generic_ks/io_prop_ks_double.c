/* io_prop_ks_double.c -- reads and writes KS quark propagators
   MIMD version 6
   MBW: 21 Feb 2001 -- just ASCII format for now, modified from
                      routines in ../generic/io_lat4.c
   MBW: Apr 2002 -- adding binary I/O
*/
/* This version is only for reading or writing 32 bit propagators into
   a double precision code.  It uses fsu3_vector only.  SG 10/1/03 */
/* when reading a 32-bit propagator, there must be a 32-destination,
   conversion to double precision, will have to be done elsewhere.  64-bit
   i/o can be done by using the corresponding files without _double SG */
...
/* Here only node 0 writes propagator to a serial file 
   The propagator is passed as 3 fsu3_vectors, one for each source color 
*/

void w_serial_ks(ks_prop_file *kspf, int color, field_offset src)
{
  /* kspf  = file descriptor as opened by w_serial_w_i 
     src   = field offset for propagator fsu3 vector (type fsu3_vector)  */
	  ...
	  if(currentnode==0)
	    {
	      ...
	      msg.ksv = *((fsu3_vector *)F_PT(&lattice[i],src));
	    }
	  else
	    {
...
int r_serial_ks(ks_prop_file *kspf, int color, field_offset src)
{
  ...
  for(rcv_rank=0; rcv_rank<volume; rcv_rank++)
	...
	if(destnode==0){	/* just copy su3_vector */
	  i = node_index(x,y,z,t);
	  dest = (fsu3_vector *)F_PT( &(lattice[i]), src );
	  *dest = pbuf[where_in_buf];
	}
	  ...
	  if(currentnode==0)
	    {
	      ...
	      pbuf = *( (fsu3_vector *)F_PT( &(lattice[l]), src ) );
	    }
	  else
	    {
      ...
      if(this_node==0)
	{
	  ...
	  if(destnode==0)
	    {              /* just copy su3_vector */
	      i = node_index(x,y,z,t);
	      *( (fsu3_vector *)F_PT( &(lattice[i]), src ) ) = pbuf;
	    }
	  else 
...
void w_serial_ksprop_tt( char *filename, field_offset prop0,
  ...
  for(t=0; t<nt; t++) {
	...
	if(currentnode==0){ 
	  ...
	  i = node_index(x,y,z,t);
	  s = &(lattice[i]);
	  proppt[0] = (fsu3_vector *)F_PT(s,prop0);
	  proppt[1] = (fsu3_vector *)F_PT(s,prop1);
	  proppt[2] = (fsu3_vector *)F_PT(s,prop2);
	  for(a=0; a<3; a++){
	    for(b=0; b<3; b++){
...
/*  The following: restore_ksprop_ascii and save_ksprop_ascii are
    depreciated in favor of [w/r]_ascii_ks_i, [w/r]_ascii_ks, and
    [w/r]_ascii_ks_f
*/
