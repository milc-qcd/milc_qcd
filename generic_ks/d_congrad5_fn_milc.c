  ...
  while(1) {
	...
	*/
	...
	if(iteration == 0){
	  ...
	  /* iteration and total_iters counts number of multiplications by M_adjoint*M */
	  iteration++;
	  total_iters++; 
	...
	if(iteration == 1)
	  node0_printf("iter=%d, rsq/src= %e, relrsq= %e, pkp=nan\n",
		       iteration,qic->final_rsq,qic->final_relrsq);fflush(stdout);
#endif
