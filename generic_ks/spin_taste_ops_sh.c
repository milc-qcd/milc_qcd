...
spin_taste_op_links(int index, int r0[], su3_vector *dest, 
  ...
  case pion05:
    ...
    /* Here the operator is 1 x 1 times (-1)^(x+y+z+t) (a scalar).                                                                                                                         It has the other parity state gamma_0 gamma_5 x gamma_0 gamma_5                                                                                                                     (a pion) -CD */
    general_spin_taste_op( G1, G1, r0, dest, src, NULL);
    break;
#ifndef NO_GAUGE_FIELD
  case pioni5:
    /* "Multiply by" the one link pion operator.                                                                                                                                           "pi_i5" or gamma_5 x gamma_i gamma_5 */
    /* Need a minus sign for backward compatibility */
    /* operator is (-)^fdir, another (-1)^(x+y+z+t) for antiquark,                                                                                                                         so multiply by (-1)^(x+y+z+t) (-)^fdir */
    char myname[] = "mult_pioni5_field";
    enum gammatype g5i = dir2g5i(ZUP, myname);
