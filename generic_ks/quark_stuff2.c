...
#define uncompress_anti_hermitian_for_inline( mat_antihermit, mat_su3 ) {\
        Real _temp1;\
	...
	_temp1=(mat_antihermit)->m01.real;\
	(mat_su3)->e[0][1].real=_temp1;\
	(mat_su3)->e[1][0].real= -_temp1;\
	(mat_su3)->e[1][0].imag=(mat_antihermit)->m01.imag;\
	(mat_su3)->e[0][2].imag=(mat_antihermit)->m02.imag;\
	_temp1=(mat_antihermit)->m02.real;\
	(mat_su3)->e[0][2].real=_temp1;\
	(mat_su3)->e[2][0].real= -_temp1;\
	(mat_su3)->e[2][0].imag=(mat_antihermit)->m02.imag;\
	(mat_su3)->e[1][2].imag=(mat_antihermit)->m12.imag;\
	_temp1=(mat_antihermit)->m12.real;\
	(mat_su3)->e[1][2].real=_temp1;\
	(mat_su3)->e[2][1].real= -_temp1;\
...
#define make_anti_hermitian_for_inline( m3, ah3 ) {\
  Real _temp,_temp2;\
	\
	_temp =\
	    ((m3)->e[0][0].imag + (m3)->e[1][1].imag);\
	_temp2 = _temp + (m3)->e[2][2].imag;\
	_temp = _temp2*0.33333333;\
	(ah3)->m00im = (m3)->e[0][0].imag - _temp;\
	(ah3)->m11im = (m3)->e[1][1].imag - _temp;\
	(ah3)->m22im = (m3)->e[2][2].imag - _temp;\
	_temp = (m3)->e[0][1].real - (m3)->e[1][0].real; (ah3)->m01.real = _temp*0.5;\
	_temp = (m3)->e[0][2].real - (m3)->e[2][0].real; (ah3)->m02.real = _temp*0.5;\
	_temp = (m3)->e[1][2].real - (m3)->e[2][1].real; (ah3)->m12.real = _temp*0.5;\
	_temp = (m3)->e[0][1].imag + (m3)->e[1][0].imag; (ah3)->m01.imag = _temp*0.5;\
	_temp = (m3)->e[0][2].imag + (m3)->e[2][0].imag; (ah3)->m02.imag = _temp*0.5;\
	_temp = (m3)->e[1][2].imag + (m3)->e[2][1].imag; (ah3)->m12.imag = _temp*0.5;\
\
}
#else /* External versions */
#define mult_su3_mat_hwvec_for_inline( mat, src, dest ) mult_su3_mat_hwvec( mat, src, dest ) 
#define uncompress_anti_hermitian_for_inline( mat_antihermit, mat_su3 ) uncompress_anti_hermitian( mat_antihermit, mat_su3 )
#define su3_projector_for_inline( a, b, dest ) su3_projector( a, b, dest )
#define make_anti_hermitian_for_inline( m3, ah3 ) make_anti_hermitian( m3, ah3 )
#endif
	...
	if( GOES_FORWARDS(lastdir) ){
      ...
      FORALLSITES(i,s){
        if( ilink<length && GOES_FORWARDS(dir) ){
          uncompress_anti_hermitian_for_inline( &(s->mom[dir]), &tmat2 );
	  ...
	  if( s->parity==EVEN )scalar_mult_add_su3_matrix(
	    ...
	    &tmat2, &tmat, -coeff, &tmat2 );
          make_anti_hermitian_for_inline( &tmat2, &(s->mom[dir]) );
        }
	if( ilink>0 && GOES_BACKWARDS(lastdir) ){
          uncompress_anti_hermitian_for_inline( &(s->mom[OPP_DIR(lastdir)]), &tmat2 );
	  su3_projector_for_inline( &(hw_tmp0[i].h[0]), &(hw_tmp0[i].h[1]), &tmat );
	  if( s->parity==EVEN )scalar_mult_add_su3_matrix(
	    &tmat2, &tmat, -coeff, &tmat2 );
	  else		 scalar_mult_add_su3_matrix(
	    &tmat2, &tmat,  coeff, &tmat2 );
          make_anti_hermitian_for_inline( &tmat2, &(s->mom[OPP_DIR(lastdir)]) );
        }
      }
      lastdir = dir;
...
/* J.O. C.D. 3/04 Copied backward links for optimization and 
...
/* Indices for tracking message tags for hwvecs */
#define  mt_temp_x    0
#define  mt_Pmu       1
#define  mt_Pnumu     2
#define  mt_Prhonumu  3
#define  mt_P7        4
#define  mt_P5        5          /* Require 8 of these */
#define  mt_P3        13         /* Require 8 of these */
#define  MT_MAX       mt_P3+8

su3_matrix *backlink[4];
...
void eo_fermion_force_3f( Real eps, int nflav1, field_offset x1_off, 
  ...
  for(i = 0; i < MT_MAX; i++)for(mu = 0; mu < 8; mu++){
    mt[i][mu] = NULL;
  }
  ...
  for(i = 0; i < MT_MAX; i++)for(mu = 0; mu < 8; mu++){
    char **pt;
    pt = (char **)malloc(sites_on_node*sizeof(char *));
    if(pt == NULL){
  ...
  FORALLUPDIR(dir){
    ...
    if(pt == NULL){
      printf("eo_fermion_force_3f: No room for backlink\n");
      terminate(1);
    }
    backlink[dir] = pt;
  }
  
  /* Gather backward links to temporary field */
  FORALLUPDIR(dir){
  ...
  FORALLUPDIR(dir){
    ...
    FORALLSITES(i,s){
      su3_adjoint((su3_matrix *)gen_pt[dir][i], backlink[dir]+i );
    }
  ...
  for(mu=0; mu<8; mu++)
    {
      u_shift_hw_fermion(temp_x, Pmu, OPP_DIR(mu), &mt[mt_temp_x][OPP_DIR(mu)], 
			 hw_pt[mt_temp_x][OPP_DIR(mu)]);
      for(sig=0; sig<8; sig++) if( (sig!=mu)&&(sig!=OPP_DIR(mu)) )
	{
	  u_shift_hw_fermion(Pmu, P3[sig], sig,
			 &mt[mt_Pmu][sig], hw_pt[mt_Pmu][sig]);
	  if(GOES_FORWARDS(sig))
	    {
	      /* Add the force F_sig[x+mu]:         x--+             *
	       *                                   |   |             *
      ...
      for(nu=0; nu<8; nu++) if( (nu!=mu)&&(nu!=OPP_DIR(mu)) )
	{
	  u_shift_hw_fermion(Pmu, Pnumu, OPP_DIR(nu), &mt[mt_Pmu][OPP_DIR(nu)], 
			 hw_pt[mt_Pmu][OPP_DIR(nu)]);
	  for(sig=0; sig<8; sig++) if( (sig!=mu)&&(sig!=OPP_DIR(mu)) &&
				       (sig!=nu)&&(sig!=OPP_DIR(nu)) )
	    {
	      u_shift_hw_fermion(Pnumu, P5[sig], sig, &mt[mt_Pnumu][sig],
				 hw_pt[mt_Pnumu][sig]);
	      if(GOES_FORWARDS(sig))
		{
		  /* Add the force F_sig[x+mu+nu]:      x--+             *
	  ...
	  for(rho=0; rho<8; rho++) if( (rho!=mu)&&(rho!=OPP_DIR(mu)) &&
				       (rho!=nu)&&(rho!=OPP_DIR(nu)) )
	    {
	      u_shift_hw_fermion(Pnumu, Prhonumu, OPP_DIR(rho), 
				 &mt[mt_Pnumu][OPP_DIR(rho)],
				 hw_pt[mt_Pnumu][OPP_DIR(rho)] );
	      for(sig=0; sig<8; sig++) if( (sig!=mu )&&(sig!=OPP_DIR(mu )) &&
					   (sig!=nu )&&(sig!=OPP_DIR(nu )) &&
					   (sig!=rho)&&(sig!=OPP_DIR(rho)) )
		{
		  /* Length 7 paths */
		  u_shift_hw_fermion(Prhonumu, P7, sig, &mt[mt_Prhonumu][sig],
				     hw_pt[mt_Prhonumu][sig] );
		  if(GOES_FORWARDS(sig))
		    {
		      /* Add the force F_sig[x+mu+nu+rho]:  x--+             *
		       *                                   |   |             *
		  ...
		  /* Add the force F_rho the 2(4) link in the path: +     */
		  u_shift_hw_fermion(P7, P7rho, rho, &mt[mt_P7][rho],
				     hw_pt[mt_P7][rho]);
	  ...
	  for(sig=0; sig<8; sig++) if( (sig!=mu)&&(sig!=OPP_DIR(mu)) &&
				       (sig!=nu)&&(sig!=OPP_DIR(nu)) )
	    {
	      /* Length 5 paths */
	      /* Add the force F_nu the 1(3) link in the path: -     */
	      u_shift_hw_fermion(P5[sig], P5nu, nu, &mt[mt_P5+sig][nu],
				 hw_pt[mt_P5+sig][nu]);
      ...
      /* Now the Lepage term... It is the same with 5-link paths with
	 nu=mu and FiveSt=Lepage. So Pnumu is really Pmumu */
      u_shift_hw_fermion(Pmu, Pnumu, OPP_DIR(mu), &mt[mt_Pmu][OPP_DIR(mu)],
					     hw_pt[mt_Pmu][OPP_DIR(mu)] );
      for(sig=0; sig<8; sig++) if( (sig!=mu)&&(sig!=OPP_DIR(mu)) )
	{
	  u_shift_hw_fermion(Pnumu, P5[sig], sig, &mt[mt_Pnumu][sig],
			     hw_pt[mt_Pnumu][sig]);
	  ...
	  /* Add the force F_nu the 1(3) link in the path: -     */
	  u_shift_hw_fermion(P5[sig], P5nu, mu, &mt[mt_P5+sig][mu],
			     hw_pt[mt_P5+sig][mu]);
	  ...
	  /* Add the force F_mu the 0(2) link in the path: +     */
	  if(GOES_FORWARDS(mu)) 
	    u_shift_hw_fermion(P3[sig], P3mu, mu, &mt[mt_P3+sig][mu],
			       hw_pt[mt_P3+sig][mu]);
	  /* The above shift is not needed if mu is backwards */
      ...
      if(GOES_BACKWARDS(mu))/* Do only the forward terms in the Dslash */
	{
	  ...
	  /* Popmu is a backward shift */
	  u_shift_hw_fermion(temp_x, Popmu, mu, &mt[mt_temp_x][mu],
			     hw_pt[mt_temp_x][mu]);
	  /* The Naik */
	  /* link no 1: - */
	  ...
	  u_shift_hw_fermion(Pnumu, Pmumumu, OPP_DIR(mu), 
			     &mt[mt_Pnumu][OPP_DIR(mu)],
			     hw_pt[mt_Pnumu][OPP_DIR(mu)] );
	  /* link no 0: + */
  ...
  /* Cleanup gathers */
  for(i = 0; i < MT_MAX; i++)for(mu = 0; mu < 8; mu++)
    if(mt[i][mu] != NULL)cleanup_gather(mt[i][mu]);
...
void u_shift_hw_fermion(half_wilson_vector *src, 
  ...
  if(*mtag == NULL)
    *mtag = start_gather_from_temp(src, sizeof(half_wilson_vector), 
				  dir, EVENANDODD, gen_pt);
  else
    restart_gather_from_temp(src, sizeof(half_wilson_vector), 
			     dir, EVENANDODD, gen_pt, *mtag);
  wait_gather(*mtag);

  if(GOES_FORWARDS(dir)) /* forward shift */
    {
      FORALLSITES(i,s)
	mult_su3_mat_hwvec_for_inline(&(s->link[dir]),
			   (half_wilson_vector *)(gen_pt[i]), &(dest[i]));
    }
  else /* backward shift */
    {
      FORALLSITES(i,s)
	mult_su3_mat_hwvec_for_inline(backlink[OPP_DIR(dir)] + i,
			   (half_wilson_vector *)(gen_pt[i]), &(dest[i]));
    }
}
