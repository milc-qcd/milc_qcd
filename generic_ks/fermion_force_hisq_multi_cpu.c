 ...
 * CD   5/5/2024 Split out from fermion_force_hisq_multi.c 
 */

/* External entry point

   fermion_force_multi_hisq_cpu
...
#ifndef FN
#error "Requires FN type actions"
#endif
					...
					fermion_links_t *fl)
{
  ...
  node0_printf("\n\n INSIDE fn_fermion_force_multi_hisq_wrapper_mx()\n\n"); // YT 20241203
...
void fermion_force_multi_hisq_cpu( info_t *info, int prec, Real eps, Real *residues,
  ...
  if(prec != MILC_PRECISION){
    node0_printf("fermion_force_multi_hisq_cpu: WARNING, precision requests not supported. Using %d.\n",
		 MILC_PRECISION);
  }

#if defined(HISQ_FF_MULTI_WRAPPER)
  fn_fermion_force_multi_hisq_wrapper_mx( info, eps, residues, multi_x, nterms, fl);
#else
  fn_fermion_force_multi_hisq_mx( info, eps, residues, multi_x, nterms, fl);
#endif
}
