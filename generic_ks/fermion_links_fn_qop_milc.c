...
QOP_FermionLinksAsqtad *
  ...
  raw_fat_links  = create_raw_G_from_field_links(t_fl,EVENANDODD);
  free(t_fl);
  if(raw_fat_links == NULL)terminate(1);

  t_ll = create_longlinks_qop_milc( &llinfo, coeffs, gauge);
  raw_long_links = create_raw_G_from_field_links(t_ll,EVENANDODD);
  free(t_ll);
  if(raw_long_links == NULL)terminate(1);
  
  lqop = QOP_asqtad_create_L_from_raw((Real **)raw_fat_links, 
				      (Real **)raw_long_links,
				      QOP_EVENODD);
