...
------SY Entry points
    f_meas_current_diff                                                                                                                                                                 f_meas_current 
    f_meas_current_multi_diff <- use iterated single-mass inverter if EIGMODE; otherwise use multi-mass inverter
    f_meas_current_multi      <- use iterated single-mass inverter if EIGMODE; otherwise use multi-mass inverter 
    if EIGMODE
      if BLOCKCG
        f_meas_current_multi_diff_eig
	f_meas_current_multi_eig   
...
/* For EIGMODE: 
   Otherise:    Entry point for multiple masses.  Uses the multimass inverter
 */
...
f_meas_current_multi( int n_masses, int nrand, int nwrite, int thinning,
  ...
  for(jrand = 0; jrand < nrand; jrand++){
    ...
    for(ex=0;ex<d;ex++)for(ey=0;ey<d;ey++)for(ez=0;ez<d;ez++)for(et=0;et<d;et++)if((ex+ey+ez+et)%2==0){
	    ...
	    /* M_inv_gr = M^{-1} gr */
#ifdef EIGMODE /* (different random source for each mass) */
	    total_iters += mat_invert_multi( gr, M_inv_gr, ksp, n_masses, qic, fn_multi );
#endif
	    
	    for(j = 0; j < n_masses; j++){

#ifndef EIGMODE /* (same random source for each mass) */
	      clear_v_field(M_inv_gr); 
